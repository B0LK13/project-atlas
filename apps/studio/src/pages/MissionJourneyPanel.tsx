...
export function MissionJourneyPanel({
  ...
  return <section className="mission-journey" aria-label="Mission workspace">
    ...
    {taskContext && <section className="task-context" aria-label="Task context"><header><h2>Task context · {taskContext.lane}</h2><ContractStatus status={String(taskContext.freshness.state ?? "UNKNOWN")} /></header><div className="journey-grid"><Panel title="Lane state"><Value value={taskContext.lane_state} /></Panel><Panel title="Candidate identity"><Value value={(taskContext.lane_state as Record<string, unknown> | undefined)?.identity} /><p>Head, tree, owner, and merge fields are source values.</p></Panel><Panel title="Verification evidence"><Value value={{ ci_status: ((taskContext.lane_state as Record<string, unknown> | undefined)?.identity as Record<string, unknown> | undefined)?.ci_status, iv_status: ((taskContext.lane_state as Record<string, unknown> | undefined)?.identity as Record<string, unknown> | undefined)?.iv_status }} /><p>CI, independent verification, and authorization remain separate.</p></Panel><Panel title="Knowledge lenses"><Value value={taskContext.knowledge} /></Panel><Panel title="Supported next step"><Value value={taskContext.next_step} /><p>Authorization: NOT_GRANTED_BY_THIS_PACKET</p></Panel><Panel title="Recovery and continuity"><Value value={taskContext.recovery} /><Value value={taskContext.continuation} /></Panel></div><Panel title="Missing inputs"><Value value={taskContext.missing} /></Panel></section>}
