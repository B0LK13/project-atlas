...
def test_provenance_swap_invalidates_evidence_digest() -> None:
    """Follow-up finding (PR #666's sibling bounded-catchup mechanism was
    independently reviewed and found to have the identical gap in its own
    evidence binding): source_package/source_directive/source_pr/
    evidence_reference -- persisted verbatim into the sealed
    TrustedAnchorRecord -- must be bound into evidence_digest, so swapping
    which work item/directive/evidence document a checkpoint claims to be
    authorized by (while leaving every topology/authorization field
    untouched) invalidates the digest."""
    current = _current_anchor()
