...
def test_read_requires_explicit_project(tmp_path: Path) -> None:
    ...
    with pytest.raises(WebDecisionsError) as exc:
        read_project_decisions(vault, "a" * 80)
...
def test_empty_project_is_unknown_not_authority(tmp_path: Path) -> None:
    ...
    assert report["honesty"]["decisions_are_authority"] is False
...
def test_http_decisions_unknown_and_writes_forbidden(tmp_path: Path) -> None:
    vault = tmp_path / "v"
    vault.mkdir()
    ...
    try:
        ...
        status, traversal = _http_json(
            str(host), int(port), hdrs, "/v1/decisions?project=../escape"
        )
        assert status == 400
