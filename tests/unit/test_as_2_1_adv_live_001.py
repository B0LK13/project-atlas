...
from project_atlas.authz import AuthzError, default_operator
...
def test_adv_default_denies_vault_write() -> None:
    with pytest.raises(AuthzError, match="vault.write"):
...
def test_adv_oai_poc_rate_limit_status_honest(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    vault = tmp_path / "v"
    vault.mkdir()
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test-not-a-real-key")

    def boom(*, prompt: str, model: str, timeout_s: float = 60.0):
