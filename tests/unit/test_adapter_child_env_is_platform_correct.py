...
def _profile(**overrides: Any) -> AgentProfile:
    payload: dict[str, Any] = {
        ...
        "adapter_options": {"argv": [sys.executable]},
    }
...
def test_a_real_child_can_do_the_work_the_fixture_workers_do(tmp_path: Path) -> None:
    ...
    # Compared case-folded on purpose: ``os.environ`` upper-cases every key on
    # nt, so the child reports SYSTEMROOT for a name this list spells
    # SystemRoot. An exact comparison here would fail on Windows for a child
    # that received the variable perfectly well -- the same case trap the
    # repair itself is about, one level up.
    child_names = {name.casefold() for name in report["env_names"]}
    for name in base_env_names(os.name):
        if name in os.environ:
            assert name.casefold() in child_names, (
...
@pytest.mark.skipif(
    os.name != "nt",
    reason=(
        "the negative arm is only meaningful on Windows: on POSIX the old "
        "construction was already correct, which is the whole point"
    ),
)
def test_the_old_construction_breaks_a_real_child_on_windows(tmp_path: Path) -> None:
    """The negative arm, on the real platform rather than in a simulation.

    Everything else here measures the repair. Nothing else shows the defect
    *failing* on Windows, because the probe worker does not exist at 2ad290d5
    and so cannot be run against it -- the missing measurement Agent 8 named
    in the clean-room review of this candidate.

    ``os_name="posix"`` reconstructs 2ad290d5's behaviour exactly: the common
    tuple, matched exact-case, which is what ``BASE_ENV_NAMES`` was before the
    platform branch existed. Running the same probe under it on a Windows host
    is the control.

    It is written to be falsifiable. If a child deprived of ``SystemRoot``,
    ``TEMP``/``TMP`` and ``USERPROFILE`` can in fact do all three of these
    things on Windows, this test fails and the causal story in the commit
    message is wrong -- which is what a control is for. It asserts that at
    least one step breaks rather than naming which, because the honest claim
    is that the old environment is inadequate, not that a particular Windows
    utility fails in a particular way.
    """
    out = tmp_path / "probe-old.json"
    profile = _profile(env_allowlist=["ATLAS_ENVPROBE_OUT"])
    old_env = build_child_env(
        profile, extra={"ATLAS_ENVPROBE_OUT": str(out)}, os_name="posix"
    )
    assert "SYSTEMROOT" not in {name.casefold().upper() for name in old_env}, (
        "the reconstruction is wrong: the old construction never forwarded "
        f"SystemRoot, yet this env has {sorted(old_env)}"
    )
    ...
    if not out.is_file():
        # The child could not start, or could not import, or could not write.
        # That is the strongest possible form of the negative result, not an
        # error in the test.
        return
    ...
    steps = report["steps"]
    outcomes = {
        name: (steps[name]["ok"], steps[name].get("value"))
        for name in ("path_home", "pid_is_alive_self", "process_start_identity_self")
    }
    degraded = [
        name
        for name, (ok, value) in outcomes.items()
        if not ok or value in (False, "unknown")
    ]
    assert degraded, (
        "a child built the old way managed every operation the repair is "
        "premised on, so missing SystemRoot/TEMP/USERPROFILE does NOT explain "
        f"the 008 failures. outcomes={outcomes} exit={completed.returncode} "
