...
def test_strict_schema_rejects_minimal_fixture_intent():
    packet = ms.build_mission_session(
        intent=_intent(), decision=_decision(), strict_schema=True, clock=clock
    )
    assert packet["session_state"] != ms.SESSION_CONFIRMED_SUCCESS
    assert packet["lifecycle"]["continuity"]["state"] in {
        ic.MALFORMED,
        ic.CORRUPT,
    }


def test_strict_schema_accepts_built_intent():
    from atlas_studio.action_intent import build_ownership_claim_intent

    intent = build_ownership_claim_intent(
        agent_id="agent-alpha",
        lane="pr/788",
        source_mc_fingerprint="a" * 64,
        clock=clock,
    )
    decision = _decision(
    ...
    packet = ms.build_mission_session(
        intent=intent, decision=decision, strict_schema=True, clock=clock
    )
    assert packet["session_state"] == ms.SESSION_CONFIRMED_SUCCESS
