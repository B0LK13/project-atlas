"""AS-CODER-ALPHA-OVERVIEW-WEB-001 — #/overview project routing + live-failure honesty."""
...
HOOK = WEB / "src" / "hooks" / "useLiveOverview.ts"
PAGE = WEB / "src" / "pages" / "production" / "OverviewPage.tsx"
...
def test_overview_route_and_nav() -> None:
    app = APP.read_text(encoding="utf-8")
    nav = NAV.read_text(encoding="utf-8")
    assert 'path="/overview"' in app
    assert '{ to: "/overview", label: "Overview" }' in nav
    assert '"/overview"' in nav


def test_overview_page_follows_project_query() -> None:
    text = PAGE.read_text(encoding="utf-8")
    assert "useSearchParams" in text
    assert 'params.get("project")' in text
    assert "useLiveOverview(projectId)" in text
    assert "overview-project" in text
    assert "UNKNOWN — overview project does not match selected project" in text
    assert "overview≠authority" in text
    assert "unknown≠healthy" in text


def test_overview_hook_does_not_label_live_failure_as_demo() -> None:
...
def test_explicit_query_uses_live_hook() -> None:
    ...
    assert "liveApiFetch(`/v1/overview?project=${encodeURIComponent(projectId)}`)" in hook
