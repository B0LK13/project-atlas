...
def test_missing_vault_fails_closed(tmp_path: Path) -> None:
    with pytest.raises(WebMissionReadError, match="mission-read-vault-missing"):
        read_mission_view(tmp_path / "absent")


def test_empty_vault_is_empty_not_healthy(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    vault.mkdir()
    view = read_mission_view(vault)
    assert view["package_id"] == PACKAGE_ID
    assert view["status"] == "EMPTY"
    assert view["available"] is False
    assert view["reason_code"] == "EMPTY_MISSION_VIEW"
    ...
    assert view["honesty"]["mission_is_authority"] is False
    ...
    assert view["honesty"]["mission_state_written"] is False
    ...
    text = render_mission_text(view)
    assert "[EMPTY]" in text
    assert "[HEALTHY]" not in text
    assert "MISSION != AUTHORITY" in text
...
def test_projects_without_ops_snapshot_are_unknown_not_healthy(
    ...
    assert view["available"] is False
    assert view["reason_code"] == "UNKNOWN_MISSION_VIEW"
...
def test_present_ops_snapshot_projects_view_not_authority(tmp_path: Path) -> None:
    ...
    assert view["reason_code"] == "MISSION_VIEW_PROJECTED"
...
def test_repeated_read_is_idempotent(tmp_path: Path) -> None:
    ...
    vault.mkdir()
    _add_project(vault, "harbor-api")
    first = read_mission_view(vault)
    second = read_mission_view(vault)
    assert first == second
    encoded = json.dumps(first, indent=2, sort_keys=True)
    assert encoded == json.dumps(second, indent=2, sort_keys=True)


def test_wrap_matches_existing_get_v1_mission(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    vault.mkdir()
    _add_project(vault, "harbor-api")
    wrapped = read_mission_view(vault)
    existing = build_mission_view(vault)
...
def test_vault_bind_does_not_import_sibling_projects(tmp_path: Path) -> None:
    ...
    left_view = read_mission_view(left)
    right_view = read_mission_view(right)
...
def test_appservice_mission_view(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    vault.mkdir()
    service = open_app_service(vault)
    view = service.mission_view()
    assert view["package_id"] == PACKAGE_ID
    assert view["status"] == "EMPTY"
    assert view["honesty"]["mission_state_written"] is False
...
def test_cli_json_report_and_show(
    ...
    assert main(["mission", "show", "--vault", str(vault), "--json"]) == EXIT_OK
...
def test_cli_report_does_not_write(tmp_path: Path) -> None:
    ...
    vault.mkdir()
    before = _snapshot(vault)
    assert main(["mission", "report", "--vault", str(vault)]) == EXIT_OK
    assert _snapshot(vault) == before
...
def test_cli_mission_names_are_free() -> None:
    ...
    parser = build_parser()
    report = parser.parse_args(["mission", "report", "--vault", "/tmp/vault"])
    assert report.command == "mission"
    assert report.mission_command == "report"
    show = parser.parse_args(["mission", "show", "--vault", "/tmp/vault"])
    assert show.mission_command == "show"


def test_cli_mission_help_is_ascii(capsys: pytest.CaptureFixture[str]) -> None:
...
def test_mcp_tool_is_allow_listed() -> None:
    ...
    assert "atlas.mission.write" not in listing["tools"]
...
def test_mcp_empty_vault_is_empty(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    vault.mkdir()
    report = invoke_mcp_tool(vault, "atlas.mission.read")
...
def test_mcp_args_and_write_keys_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(McpServerError, match="mcp-request-forbidden-key"):
        handle_mcp_request_line(
            vault,
            json.dumps({"tool": "atlas.mission.read", "args": {"project": "x"}}),
        )
    with pytest.raises(McpServerError, match="mcp-request-forbidden-key"):
        handle_mcp_request_line(
            vault,
            json.dumps({"tool": "atlas.mission.read", "write": True}),
        )
    assert _snapshot(vault) == before
...
def test_api_existing_mission_projection_unchanged(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    vault.mkdir()
    _add_project(vault, "harbor-api")
    ...
    try:
        ...
        with urlopen(
            Request(f"http://{host}:{port}/v1/mission/report", headers=auth),
            timeout=2,
        ) as resp:
        ...
        assert report["honesty"]["WRITE_APPLIED"] is False
        assert report["honesty"]["mission_is_authority"] is False
        assert report["honesty"]["view_is_truth_core"] is False
    finally:
        server.shutdown()


def test_api_mission_report_is_get_only(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    vault.mkdir()
    server = serve_api(vault, host="127.0.0.1", port=0)
    host, port = server.server_address[:2]
    ...
    try:
        auth = session_credentials(server).auth_headers()
        req = Request(
            f"http://{host}:{port}/v1/mission/report",
            data=b"{}",
...
def test_d149_and_atlas3_untouched() -> None:
    ...
    if atlas3.exists():
        for path in atlas3.rglob("*"):
            if path.is_file():
                assert "AS-CODER-ALPHA-MISSION-READ-001" not in path.read_text(
