...
INBOX != AUTHORITY. No implicit portfolio-all. Layer B writes stay 0.
"""
...
def test_list_is_project_scoped_and_deterministic(tmp_path: Path) -> None:
    ...
    assert "portal-app" not in json.dumps(report["items"])
    assert "harbor-api" not in json.dumps(leaked["items"])
...
def test_cli_inbox_list_json_and_human(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["layer_b_writes"] == 0
