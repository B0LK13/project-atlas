...
def _resolve_diff_base(*, root: Path = ROOT, env: Mapping[str, str] | None = None) -> str:
    ...
    Prefers the authoritative GitHub ``pull_request`` event base SHA when
    running in that context (works correctly under any checkout depth,
    including the hosted CI default of a single-commit shallow fetch).
    Falls back to ``origin/main`` for local/full-clone use, but fails
    closed (raises) if that ref cannot be resolved either, rather than
    letting the caller silently treat an unresolvable base as "nothing
    changed".
    """
    ...
    if resolve.returncode != 0:
        raise DemoIsolationGuardError(
            "not running in a GitHub Actions pull_request context and "
            "origin/main is not resolvable locally -- cannot determine "
