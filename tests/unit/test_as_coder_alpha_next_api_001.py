...
def test_unknown_project_is_malformed_not_invented(tmp_path: Path) -> None:
    vault = tmp_path / "v"
    vault.mkdir()
...
def test_empty_project_is_unknown_not_command(tmp_path: Path) -> None:
    vault = tmp_path / "v"
    (vault / "projects" / "empty-proj").mkdir(parents=True)
...
def test_http_next_scope_isolation_and_writes(tmp_path: Path) -> None:
    ...
    try:
        ...
        dumped_harbor = json.dumps(harbor, sort_keys=True)
        assert "portal-app" not in dumped_harbor
