...
def test_d148_evidence_rejects_demo_path_even_when_bound(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """D-149R6: a fingerprint-bound cert on a demo/ path is not current."""
    (tmp_path / "dev-ai" / "demo").mkdir(parents=True)
    estate = _init_repo(tmp_path / "dev-ai" / "demo")
    head = subprocess.run(
        ["git", "rev-parse", "HEAD"],
        cwd=estate,
        check=True,
    ...
    monkeypatch.setenv("AUTHENTIC_ESTATE_ROOT", str(estate))
    evidence = _bound_evidence(estate, head=head, fingerprint=estate_fingerprint(estate))
    assert run_estate_preflight(estate).preflight_pass is False
    assert d148_evidence_applies(evidence, head, estate) is False
