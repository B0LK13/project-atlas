...
def test_no_runtime_conflict_projections_mutation() -> None:
    """Prep must not touch conflict_projections or other src surfaces."""
    ...
    assert "src/project_atlas/conflict_projections.py" not in changed
    for name in changed:
        assert not name.startswith("src/"), name
        assert name.startswith("docs/atlas-2.2/conflict-ux/") or name == (
            "tests/unit/test_as_2_2_conflict_ux_prep_001.py"
        ), name
