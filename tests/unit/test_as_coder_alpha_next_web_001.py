"""AS-CODER-ALPHA-NEXT-WEB-001 — #/next project routing + live-failure honesty."""
...
HOOK = WEB / "src" / "hooks" / "useLiveNext.ts"
PAGE = WEB / "src" / "pages" / "production" / "NextPage.tsx"
...
def test_next_route_and_nav() -> None:
    app = APP.read_text(encoding="utf-8")
    nav = NAV.read_text(encoding="utf-8")
    assert 'path="/next"' in app
    assert '{ to: "/next", label: "What Next" }' in nav
    assert '"/next"' in nav


def test_next_page_follows_project_query() -> None:
    text = PAGE.read_text(encoding="utf-8")
    assert "useSearchParams" in text
    assert 'params.get("project")' in text
    assert "useLiveNext(projectId)" in text
    assert "next-project" in text
    assert "UNKNOWN — next project does not match selected project" in text
    assert "next≠authority" in text
    assert "next≠command" in text


def test_next_hook_does_not_label_live_failure_as_demo() -> None:
...
def test_explicit_query_uses_live_hook() -> None:
    ...
    assert "liveApiFetch(`/v1/next?project=${encodeURIComponent(projectId)}`)" in hook
...
def test_next_is_not_a_command() -> None:
    text = PAGE.read_text(encoding="utf-8")
    assert "These lines are suggestions, not commands." in text
