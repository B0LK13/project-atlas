# Cross-project fabric — package index

Status: **PREP ONLY** (pre-`v2.1.0`). Docs / contract stubs / fixtures.
Production mutation: **NONE**. Gate credit: **NO**.
`ATLAS_2_1_RELEASE_CERTIFIED`: **NO**.

| Doc | Role |
|---|---|
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Estate fabric layers + truth boundaries |
| [CONTRACT.md](./CONTRACT.md) | FR/NFR stubs + operation shapes |
| [INVARIANTS.md](./INVARIANTS.md) | Hard fail-closed rules |
| [FIXTURE-PLAN.md](./FIXTURE-PLAN.md) | Fixture family reservation |
| [AS-2.2-XPROJ-CONTRACT-PREP-001.md](./AS-2.2-XPROJ-CONTRACT-PREP-001.md) | Package charter |
| [./contracts/](./contracts/) | Docs-only JSON Schema stubs |
| [./fixtures/](./fixtures/) | Synthetic sample payloads |
| [./adr/](./adr/) | Prep boundary ADR |

## Truth boundaries

```text
CROSS-PROJECT IDENTITY  ≠  AUTOMATIC AUTHORITY
CROSS-PROJECT EDGE      ≠  AUTOMATIC AUTHORITY
DUPLICATE CANDIDATE     ≠  UUID COLLAPSE
XPROJ INDEX             ≠  AS-RET-001 LEXICAL INDEX
NAME / STRING           ≠  IDENTITY
FIXTURE PASS            ≠  PILOT / RELEASE
```

## Unlock

Runtime work for `AS-2.2-XPROJ-001` remains blocked until
`ATLAS_2_2_INTELLIGENCE_IMPLEMENTATION_UNLOCKED` after `v2.1.0` certification.

## Deepen PREP

See [`AS-2.2-XPROJ-DEEPEN-PREP-001.md`](AS-2.2-XPROJ-DEEPEN-PREP-001.md).
