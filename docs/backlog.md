...
- [x] AS-CODER-ALPHA-EVENT-TOMBSTONES-READ-001 vault-scoped event-tombstone lens (`atlas event-tombstones`, `/v1/event-tombstones`, `#/event-tombstones`, `atlas.event.tombstones.read`; DELETED!=VANISHED; EMPTY!=HEALTHY; UNKNOWN!=CLEAN; LENS!=AUTHORITY; draft; MERGE_AUTHORIZATION NOT_GRANTED)
