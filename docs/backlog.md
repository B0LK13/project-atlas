...
- [x] AS-CODER-ALPHA-INCREMENTAL-CONNECT-001 no-change reconnect must not double discover+ingest (implemented on main; independent IV 2026-08-24T23:10Z PASS via `test_as_coder_alpha_incremental_connect_001` + `test_as_coder_alpha_connect_001`; MERGE already landed; not a D-149 change)
...
- [x] AS-CODER-ALPHA-DECISIONS-MCP-001 zero-arg `atlas.decisions.read` (vault-scoped; MCP≠authority; lens≠ACTIVE_GOVERNING grant)
