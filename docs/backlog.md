...
- [ ] AT3-CLI-F2 Memory CLI stale must not dump CURRENT-tagged rows (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). `atlas memory stale` bypassed the AT3-081 `stale_memories` freshness guard and reported CURRENT items as stale inventory with exit 0. Now fail-closed as `STALE_AS_CURRENT`. Does not change `stale_conflict.py` or `ingestion.py`.
