...
- [ ] AS-OBS-SIG005-001 Live promotion-failure schema must fail OPS-SIG-005 (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). `ops_health._collect_transaction_failures` read only `failures` / `items`, so the receipt ingestion actually writes (`projects[].candidates[].outcome=PROMOTION_FAILED`) reported `ok` / `observed_value=0` while `source_health` and `attention_hygiene` on the same file were ACTION_REQUIRED. Present failure evidence must not fabricate estate `healthy` once recommended signals are known. Empty list indexes and absent files stay `ok/0` and `unknown`. Does not touch `ingestion.py`. `MERGE_AUTHORIZATION = NOT_GRANTED`.
