...
## Atlas Studio (epic #746 / AS-STUDIO-INTAKE-20260908-01)

Canonical package: `docs/atlas-3/studio/`.
Honesty: `PREP != IMPLEMENTED` · `STUDIO_UI != AUTHORITY` ·
`IMPLEMENTED != MERGED_TO_MAIN` (coordination F1–F16 tip) ·
...
- [ ] AS-CORE-VALIDATE-RELLINK-001 **WITHDRAWN as a duplicate** — `atlas validate` exits 1 on this repository (`claims.md -> OPENAI-MCP-DESIGN.md`). Root cause is NOT a relative link: the link sits inside a **code span** in compiled claim text, and the validator does not mask inert Markdown regions. Already owned and implemented by **#700**; A/B on one identical vault: main `b87b4a22` exit 1, #700 `16e654a8` exit 0 (838 files). Reproduction posted to #700; no competing implementation opened.
