...
_Status: **IMPLEMENTED — NOT YET MERGED, NOT PRODUCTION-READY** (this PR, branch `feat/phase2a-specification-backed-origination`; see `docs/adr/ADR-033-specification-backed-work-origination.md` and WORKLOG "AS-ORIGIN-001"). Bridges the two already-shipped, previously-disconnected pipelines ADR-033 identifies: deterministic claim extraction (`knowledge_compiler.py`/`ingest`, producing `Claim` objects at `state/claims/<project_id>.json`) and the Living Project Roadmap lens (`project_roadmap.py`, which only ever reads a structured fenced record at `projects/<id>/roadmap.md`). New module `orchestration/origination.py` (autonomy-adjacent, not under `autonomy/`) reads persisted claims, groups them by `(project_id, subject)`, applies a deterministic evidence-quorum policy gate (>=1 authoritative-intent signal AND >=1 corroborating-acceptance signal for the *same* subject -- never merely the same project), rejects contradictory/already-implemented/superseded/authority-floor-failing evidence, and -- only when the resulting proposal is `VALID` -- writes a roadmap record in the exact `roadmap_items[]` shape `project_roadmap.py` already parses, atomically. `project_roadmap.py` itself is **not modified**. Phase 2A-1 only, per ADR-033's Phasing section: does **not** touch the governor, lease/dispatch machinery, or any file under `orchestration/autonomy/` -- that integration is Phase 2A-2, a separate later PR._

Honesty (mandatory):

- `ORIGINATION_PROPOSAL_IS_COMMAND = NO`
- `EXECUTION_READY_WHEN_OWNER_HELD = NO` (an `OWNER_HELD` proposal is written, if at all, as an explicit `BLOCKED` roadmap item carrying an owner-authorization blocker reason -- never silently surfaced as ready)
- `MODEL_DECIDES_POLICY = NO` (extraction, correlation, and policy are pure deterministic Python; no model call ever decides pass/fail)
- `GOVERNOR_INTEGRATION = NOT YET IMPLEMENTED` (Phase 2A-2)
- `LEASE_DISPATCH_INTEGRATION = NOT YET IMPLEMENTED` (Phase 2A-2)
- `EXTRACTION_PIPELINE = UNMODIFIED` (this package only reads `state/claims/<project_id>.json`; it never changes what `ingest`/`knowledge_compiler.py` extracts)

- [x] AS-ORIGIN-001-001 `EvidenceSignal`/`OriginationProposal` Pydantic models per ADR-033's schema (`is_command=False`/`executable=False`/`risk_class="O1"` always; `extra="forbid"`)
- [x] AS-ORIGIN-001-002 `extract_candidate_facts` -- deterministic grouping by `(project_id, subject)`; claims from another `project_id` are excluded by construction, never by a filter that can be bypassed
- [x] AS-ORIGIN-001-003 `correlate_evidence` -- the evidence-quorum rule, correlated within one subject only (this is what makes "unrelated failing tests anywhere in the project" a negative, not a false positive), with an authority-floor and lifecycle-floor eligibility check before any claim can become a signal
- [x] AS-ORIGIN-001-004 `validate_policy` -- contradiction rejection (reuses `knowledge_compiler._conflicts`, not a second contradiction detector), `INSUFFICIENT_ACCEPTANCE_CONTRACT` handling when success criteria can't be pinned to a named test or a located acceptance clause, a static owner-gated-surface path classifier (CI/workflow, auth/security, dependency manifests unless the evidence itself specifies the dependency, migrations, deploy config)
- [x] AS-ORIGIN-001-005 Idempotent fenced-record writer/reader (`write_origination_record`/`read_origination_proposal`) round-tripping full provenance through `projects/<id>/roadmap.md`, same atomic-write convention (temp file + `os.replace`) as `project_roadmap.py`'s own `_write_atomic`
- [x] AS-ORIGIN-001-006 `run_origination(vault, project_id)` public entry point: extraction -> correlation -> policy -> conditional write; always returns the proposal (including `BLOCKED`/`INSUFFICIENT_ACCEPTANCE_CONTRACT` ones) for callers/tests to inspect without a record having been written
- [x] AS-ORIGIN-001-007 Full adversarial/negative suite (`tests/unit/test_as_origin_001.py`, 39 tests, all executed and passing): TODO-only evidence, speculative README ideas, contradictory roadmap/requirements state, already-completed work, superseded specifications, owner-blocked work, missing acceptance criteria, unrelated failing tests, stale evidence, cross-project evidence contamination, malicious/instruction-like prose, unsupported inference with no authoritative claim -- plus `NO_DUPLICATE_ORIGINATION`, `STABLE_WORK_IDENTITY`, `PROVENANCE_SURVIVES_RESTART`, `NO_CROSS_PROJECT_LEAK`, `OWNER_GATE_PRESERVED` -- plus the IV remediation regressions (PR #642 review pass): dependency-manifest gate requires eligible evidence, conflict detection scoped to the candidate project (never leaks a foreign-project value), already-completed work vetoed regardless of which field records it, skip-substring false positives ("unskipped and passing", "no longer skipped"), locator-alone is not a concrete acceptance criterion, named-test-resource requires a real test file path, legacy `items` key dropped after normalization, and stale generated roadmap items are reconciled away once their evidence stops qualifying
- [x] AS-ORIGIN-001-008 Generic acceptance against the real `D:/Atlas-Demo` showcase estates (`tests/integration/test_as_origin_001_showcase_acceptance.py`, 7 tests, real `atlas discover`/`ingest`/`build-indexes`/`validate` run against read-only copies, `D:/Atlas-Demo` itself never modified): Alpha and Beta correctly produce no origination (no evidence quorum, as expected); Gamma's real, *unmodified* `atlas ingest` also currently produces no origination quorum today -- see the open design question below -- and a supplementary, clearly-labeled test proves `run_origination`'s own policy logic (zero TASK-017/Gamma-specific code in `origination.py`'s production path) correctly recognizes Gamma's real TASK-017 evidence once that evidence exists as structured claims; plus 3 platform-independence regression tests for the estate-root absolute-path skip guard (PR #642 review pass)

**Open design question flagged for owner input (not guessed at silently; conservative/fail-closed choice made in the meantime):** ADR-033 treats ROADMAP_STATUS/WORK_PACKAGE_STATUS/TEST_RESULT claims naming a specific work item's readiness as "already-ingested project evidence." Verified directly against the real Gamma estate: the current deterministic extractor (`claim_identity.py`'s `_LINE_RULES`) requires literal `key: value` syntax (e.g. `Status: ...`) and only falls back to a whole-paragraph claim for documents whose legacy classification is exactly `"architecture"`; Gamma's real `docs/ROADMAP.md`/`docs/REQUIREMENTS.md` are prose and classify as `"validation"`, so real `atlas ingest` extracts zero claims from them today, and `run_origination` therefore correctly finds no quorum for Gamma -- fail-closed, not a defect in this package. Whether/how to enrich extraction (or add a second evidence source) for prose-shaped roadmap/requirements documents is out of Phase 2A-1's scope and is left for a separate owner decision.
