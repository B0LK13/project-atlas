...
- [ ] D-057-MARKER-YAML-CONSTRUCTOR-KEYERROR Map PyYAML ``!!bool`` constructor ``KeyError`` onto structured refusals in ``discover``, ``connect``, and origination marker/contract loaders (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). Sibling of #819 / #820 / #822. Does not touch ``ingestion.py``. Evidence: ``docs/evidence/D-057-MARKER-YAML-CONSTRUCTOR-KEYERROR.md``.
