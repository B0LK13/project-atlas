...
- [x] AS-CODER-ALPHA-INCREMENTAL-CONNECT-001 no-change reconnect must not double discover+ingest (implemented on live main `f0e0c979`; `incremental_connect.py` + connect hook + 21 unit tests PASS this cycle; operational skip only; not Truth Core authority; MERGE_AUTHORIZATION NOT_GRANTED for unrelated drafts)
