...
- [x] AS-CODER-ALPHA-INCREMENTAL-CONNECT-001 no-change reconnect must not double discover+ingest (implemented on main; night-cycle 2026-08-25T01:30Z independent IV PASS 21/21 on `f0e0c979`; MERGE_AUTHORIZATION still NOT_GRANTED for unrelated drafts)
- [x] AS-CODER-ALPHA-STATE-ATTENTION-WEB-MCP-001 first-class `/state` + `/attention` pages and vault-scoped `atlas.project-state.read` / `atlas.project-attention.read` / `atlas.roadmap.read` MCP (uses existing LIVE_API; UI≠canonical; MCP≠authority)
