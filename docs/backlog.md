...
- [ ] AS-OBSIDIAN-CAPTURE-001-F3 Literal generated markers inside a HUMAN region (OWNER DECISION REQUIRED). A literal `<!-- atlas:generated:start -->` marker inside HUMAN content is counted structurally -- the count is over the full comment string, so a bare `atlas:generated:start` is *not* affected -- and the note then fails closed with `OBSIDIAN_NOTE_CONFLICT` on every refresh. Fail-closed availability, not data loss, and the note is left untouched. Decide whether marker spellings are reserved everywhere or HUMAN payload is opaque; opaque parsing needs ownership rules and anti-forgery coverage, so simply dropping the count is unsafe.
