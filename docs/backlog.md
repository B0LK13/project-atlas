...
- [ ] AS-OBSIDIAN-CAPTURE-001-F7-R1 Contain `yaml.safe_load` constructor `KeyError` in `_existing_capture_id` (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). F7 recorded this residual and left it unfixed: `!!bool nope` raises a bare `KeyError`, not `yaml.YAMLError`, so public `retry()` escaped with a traceback. Outcome was already fail-closed (note bytes unchanged); the defect is the mechanism. Catch `(yaml.YAMLError, KeyError)` and treat the shape as unmanaged -- same `OBSIDIAN_NOTE_CONFLICT` as other hostile frontmatter. Does not widen acceptance. Does not touch `ingestion.py`. Evidence: `docs/evidence/AS-OBSIDIAN-CAPTURE-001-F7-R1-YAML-KEYERROR.md`.
