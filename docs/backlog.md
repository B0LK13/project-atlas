...
- [x] AS-CODER-ALPHA-044-HIGH D-041 Windows/adversarial HIGH truth/isolation remediations (on live main; `test_as_coder_alpha_044_d041_high.py` 20 passed / 1 env skip this cycle; not a merge grant)
...
- [x] AS-CODER-ALPHA-LENS-MCP-001 zero-arg overview/decisions/unknown/changed/architecture/inbox/captures MCP (vault-scoped; MCP!=authority; no inventory rotate; no promote; no request args)
- [x] AS-CODER-ALPHA-INCREMENTAL-CONNECT-001 no-change reconnect must not double discover+ingest (implemented on live main `f0e0c979`; unit 13 passed this cycle; MCP!=this package; MERGE of this checkbox is honesty only)
