...
- [ ] EVENT-PACKAGE-RECEIPT-YAML-KEYERROR Map PyYAML ``!!bool`` constructor ``KeyError`` onto ``PackageValidationError`` for ``receipt.yaml`` (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). Sibling of #819 / #820 / #822 / #823 / #824. Does not touch ``ingestion.py``. Evidence: ``docs/evidence/EVENT-PACKAGE-RECEIPT-YAML-KEYERROR.md``.
