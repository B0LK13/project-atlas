...
- [ ] AS-OBSIDIAN-CAPTURE-001-F8 #716's split-token near miss is pinned (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). A **pin, not a fix**: the behaviour is already correct on `main` and `src/` is byte-identical to it. The F1-F4 residual register recorded that `<!-- atlas:generated:sta rt -->` "behaves correctly but is pinned by no assertion in the F3 test file". Reproduced through `merge_protected_regions` on `7b0989a7` and re-run on `e264d599` after F6 moved the base (`protected_regions.py` is hash-identical at both, `d3fe8615`, so the two runs are one measurement -- both were done rather than assumed): four split shapes are all preserved as ordinary HUMAN prose with bytes intact. **Why it matters**: the owner policy makes marker spellings reserved everywhere, so the matcher's exactness is load-bearing in a direction that fails quietly -- a matcher made *more* tolerant does not error, it starts REFUSING ordinary human prose, leaving a note that discusses Atlas syntax permanently unmanageable with the error naming the wrong cause. Same consumer and consequence as the CRLF regression F5's verification caught and the BOM defect F7 fixed, reached from a third direction. Four cases added to the F3 NEAR_MISS corpus (7 -> 11). **Three negative controls, each run against BOTH the corpus on main and the pinned corpus**, because a control that only shows the new tests failing proves they are tests, not that they are needed: A (broadly whitespace-tolerant matching) 1 caught -> 5; B (tolerant only of a break inside the token) **0 caught, 37 passed clean** -> 3; C (tolerant only of whitespace around the colons) **0 caught, 37 passed clean** -> 1. B and C carry the weight -- both are plausible relaxations, both pass entirely undetected against main's corpus, and the pre-existing `extra-inner-spacing` case catches only A. **Not claimed**: that the corpus is complete -- the near-miss space is not enumerated, no exhaustive sweep is committed, and no coverage fraction is asserted. Evidence: `docs/evidence/AS-OBSIDIAN-CAPTURE-001-F8-SPLIT-TOKEN-PIN.md`.
