...
- [ ] AS-ASK2-UNREADABLE-CONFLICTS-001 Corrupt `review/conflicts/*.json` must not launder Ask Atlas 2 CONFLICT into KNOWN (IMPLEMENTED on `fix/as-ask2-unreadable-conflicts-known-001`; AWAITING IV; MERGE_AUTHORIZATION = NOT_GRANTED). Missing overlay directory remains absent. Mixed valid+corrupt still reports conflict.
