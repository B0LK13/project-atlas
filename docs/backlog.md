...
- [x] D-PHASE2A-1b Contain raw OSError from origination projection writes (`PROJECTION_WRITE_BLOCKED`) at all five `ProjectIdentityLock` mutators. Shared `_write_atomic` stays OSError-typed; origination CLI catches `OriginationProjectionError` only. Reproduced on `b87b4a22`. MERGE_AUTHORIZATION NOT_GRANTED.
