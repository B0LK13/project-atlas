...
- [x] AS-OBSIDIAN-CAPTURE-001-F1 Ambiguous HUMAN region identity fails closed (a name repeated **in one scope**, or a region nested inside one of its own name, is ambiguous; extraction keys blocks by name so a re-render silently dropped a human block -- now refused, note left byte-identical. Resolved structurally over region spans, not by counting names: counting refused nested distinct-name documents on their *second* render, which would have decided F2 by side effect. Sibling names are compared per scope at every depth; a top-level-only check shipped briefly as a fail-open. as of main, `graph_projections.py` still carries an independent copy of the defect, tracked as F4)
- [x] AS-OBSIDIAN-CAPTURE-001-F2 HUMAN region nesting semantics -- resolved as **structural-scope-qualified identity**: a region is identified by its ancestry scope path plus its name, so `a/x` and `b/x` are independent and preserved independently. Name-keyed last-wins collapsed them into one slot and spliced the survivor into the other container -- silent human-content loss, measured at 111 lost payloads per 1,241 merges on a randomized harness before the fix and 0 after. Same-scope repeats, self-nesting and crossed markers fail closed with the note untouched; the merge plan is resolved and validated before any byte is written. Nested distinct-name documents no longer duplicate the inner block. as of main, `graph_projections.py` still carries an independent implementation this does not reach (F4)
- [x] AS-OBSIDIAN-CAPTURE-001-F3 Literal generated markers inside a HUMAN region -- **OWNER DECISION 2026-09-07: marker spellings are RESERVED EVERYWHERE.** The exact Atlas structural marker comment is reserved syntax wherever it appears, including inside a HUMAN region; the count is over the full comment string, so a bare `atlas:generated:start`, extra-space and suffixed near-misses remain ordinary text. The rejected alternative, treating HUMAN payload as opaque, would have undone F2's nested-region structure (an inner `BEGIN HUMAN` is structure, not prose) and imported the ownership/anti-forgery obligation this entry already warned made "simply dropping the count" unsafe. Runtime already conformed: exact spellings fail closed with the note left byte-identical -- an availability defect, never data loss -- so no parser or grammar change was needed. Closed with tests pinning both halves of the policy (`tests/unit/test_as_obsidian_capture_001_f3_reserved_markers.py`, previously the permissive half was unasserted) and a diagnostic that reports observable counts and **names no cause**: `start_count > 1` is reached both by an operator-authored reserved spelling and by malformed Atlas structure, and the validator runs before regions are extracted, so it cannot know which occurred.
- [ ] AS-OBSIDIAN-CAPTURE-001-F4 Consolidate the third protected-region implementation (OWNER/IV GATED; merge NOT pre-authorized). **On main today** `graph_projections._merge_protected_regions` is still an independent name-keyed copy that does **not** delegate to `protected_regions.py`, so neither F1's ambiguity fix nor F2's scope-qualified identity reaches graph projections: same-leaf cross-scope `a/x`/`b/x` is accepted with one payload destroyed and the survivor spliced into both scopes, same-scope duplicates are accepted with last-wins loss, and self-nesting/crossed/unclosed are refused only incidentally via marker-count mismatch. **Candidate** branch `fix/graph-projections-canonical-human-region-semantics` replaces that copy with a narrow adapter onto the canonical core, translating `ProtectedRegionError` to `GraphProjectionError`, and retains exactly one disclosed graph-specific contract: a prior note with no HUMAN regions keeps its text outside the generated span, where the canonical core returns the fresh render. The claim is canonical reuse of HUMAN-region semantics only -- not general equivalence. Durable quantified baseline lives in the separate evidence lane (PR #706, deterministic seeded harness); the older "~50.5%" figure is superseded and non-reconstructible and must not be cited.
