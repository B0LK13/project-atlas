...
- [ ] OPS-HEALTH-YAML-CONSTRUCTOR-KEYERROR Map PyYAML ``!!bool`` constructor ``KeyError`` onto the existing unreadable-YAML ``None`` path in ops-health (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). Sibling of #819 / #820 / #822 / #823 / #824 / #825. Does not touch ``ingestion.py``. Evidence: ``docs/evidence/OPS-HEALTH-YAML-CONSTRUCTOR-KEYERROR.md``.
