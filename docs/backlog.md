...
- [ ] AS-OBSIDIAN-CAPTURE-001-F4 Consolidate the third protected-region implementation (OWNER/POLICY GATED). `graph_projections._merge_protected_regions` is an independent copy that does **not** delegate to `protected_regions.py`, so neither F1's ambiguity fix nor F2's scope-qualified identity reaches graph projections. Characterised and quantified in `docs/evidence/F4-GRAPH-PROJECTION-HUMAN-REGION-DIVERGENCE.md`, reproducible via `docs/scripts/f4_protected_region_divergence_harness.py`: **six** divergences (five historical drift, one intentional graph-specific contract -- graph preserves text outside the generated span when a note has no HUMAN regions, where the canonical helper returns the fresh render). At the real refresh surface four of nine shapes silently drop human bytes; on a 4,000-trial seeded harness graph loses payloads in 897 of 3,051 accepted merges (29.4% harness incidence, **not** a production prevalence rate), duplicates content in 194, and grafts spurious region structure in 108, where canonical F2 records zero on every counter. Consolidation must be a narrow adapter onto the canonical core, not a blind replacement or a fourth copy, and must not claim a single implementation until that is literally true.
