...
- [x] AS-CODER-ALPHA-PORTFOLIO-INDEX-001 vault-scoped `#/portfolio` + `atlas.portfolio.state.read` over existing `/v1/projects` + `/v1/portfolio-state` (no `/v1/portfolio` protocol; empty-arg portfolio-state remains UNSUPPORTED_SCOPE; PORTFOLIO!=authority; UI!=canonical; draft only — MERGE_AUTHORIZATION NOT_GRANTED)
