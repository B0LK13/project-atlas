...
- [x] AS-CODER-ALPHA-LENS-MCP-001 zero-arg `atlas.overview.read` / `atlas.decisions.read` / `atlas.unknown.read` / `atlas.changed.read` / `atlas.architecture.read` (vault-scoped; MCP!=authority; no inventory rotate; no request args)
