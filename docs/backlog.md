...
- [x] AS-CODER-ALPHA-INCREMENTAL-CONNECT-001 no-change reconnect must not double discover+ingest. Phantom-work correction 2026-08-28: implemented and wired on current `main` -- `connect.py` names this package directly in its own docstring and contains `evaluate_incremental_reconnect()` / `_finish_no_change_reconnect()`, called from the real `connect` flow, verified directly against the code (not stub text). Origin PR #374 is merged.
