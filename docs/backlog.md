...
- [x] AS-CODER-ALPHA-SESSION-CAPTURE-READ-001 zero-arg `atlas.session-capture.read` + `GET /v1/session-captures` + `#/session-captures` (vault-scoped; OPS_RECEIPT!=Truth Core; no record/write; distinct from conversation-capture)
