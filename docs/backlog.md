...
- [ ] AS-OBSIDIAN-CAPTURE-001-F14 The last three raw `OSError`s in `_promote` (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). `_promote` is the transactional write boundary and three failures there escaped OUTSIDE `GraphProjectionError`, so a caller catching the module's own type -- the type its docstrings tell callers to catch -- did not catch them at all. Reproduced on current `main` `b87b4a22` BEFORE changing anything, raising line attributed by walking the traceback: read-only output directory -> `PermissionError` at 620 (`staged.write_bytes`); unreadable existing target -> `PermissionError` at 616 (`path.read_bytes()`); `ENAMETOOLONG` -> `OSError` at 614 (`path.exists()`). #757 predicted 614/616/620 for this object and the prediction held exactly. Two are named verbatim in F6's residual register; the THIRD (`ENAMETOOLONG`) is recorded nowhere and was found while reproducing the other two. All pre-existing -- F11 closed the `mkdir` site one step earlier in the same loop and did not introduce these. Each guard separately load-bearing, measured by removing one at a time with the file restored between runs: unguarding the stat, the read, or the staging write each fails 2 of 8; restored 8 passed. **Three reasons, not one**, because an unstattable path, an unreadable existing note and an unwritable staging file are three different things to go and fix, and `_generated_span` already states 'the operator can act on the difference' as this module's diagnostic rule. **On fixture liveness**: the permission cases are INERT where a process reads regardless of mode (root; Windows, where `chmod(0o000)` does not block a read), and a test that silently skipped there would report green while measuring nothing -- so each fixture is PROBED and skips only with the platform named, while portable monkeypatched cases carry the containment logic everywhere. **Not claimed**: that `_promote` is now exhaustively fail-closed -- these are the three sites F6's register and this reproduction identified, the enumeration was not proved complete, and a fourth site would look exactly like these did. **IV remediations on the same carrier (still `[ ]`, still not merge-authorized):** the tautological `len({three literals})==3` test now provokes live prefixes; `had_original` no longer re-stats (the fourth raw-`OSError` site IV found); contained failures assert no staging residue; POSIX fixtures restore the measured mode.
