...
- [x] AS-CODER-ALPHA-WORKSPACE-READ-001 vault-scoped GET /v1/workspace REPORT READ (`atlas workspace report` / `GET /v1/workspace/report` / `atlas.workspace.read`; WORKSPACE!=AUTHORITY; VIEW!=TRUTH CORE; EMPTY!=HEALTHY; UNKNOWN!=HEALTHY; WRITE_APPLIED=false)
