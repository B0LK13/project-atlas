...
- [x] D-PHASE2A-2 Wiring origination into the live governed DAG/lease/dispatch loop (`orchestration/autonomy/loop.py` and friends) -- MERGED on main via #654 (`feat(orchestration): wire origination into the live governed DAG (D-PHASE2A-2)`), 2026-08-31T08:07:50Z, merge commit `818dd140`; that PR modified `src/project_atlas/orchestration/autonomy/loop.py`, the file this line names, and it is present on main. No formal-IV receipt located on the PR or the DAG event bus -- MERGED != IV_PASS; absence of a receipt is not proof that no review occurred, and this merge grants no permission to bypass current gates. Historical context retained: explicitly deferred by D-PHASE2A-1's own scoping ("Phase 2A-1 only... a separate later PR"). Both of its prior blockers are now resolved: D-PHASE2A-1a (above) is merged, and PR #642 (`feat/phase2a-specification-backed-origination`, the independent overlapping implementation) is **CLOSED as superseded** (not merged, not transplanted) -- verified before closure that merging it as-is would have been semantically broken (a real module/package namespace collision: `orchestration/origination.py` vs. the merged `orchestration/origination/` package, confirmed via an isolated Python import test that Python always resolves to the package, permanently orphaning `origination.py` and breaking its own tests), plus a duplicate `ADR-033` document. The `#643`-descendant `orchestration/origination/` package (extended by D-PHASE2A-1a/-3) is the canonical Phase 2A origination lineage. Canonical carrier was PR #654 (`feat/d-phase2a-2-origination-governor-wiring`); when this line was first written the wiring was implemented there and **not yet merged** -- it merged 2026-08-31 as `818dd140` (see the head of this line). Night-cycle 2026-08-30 remediations on that same carrier (exact-object recert required after HEAD move): completed-dependency CERTIFIED witnesses, durable lease-sequence adoption, STOPPED/NO_ELIGIBLE_WORK resume when later origination adds READY work, stale `base_pin` skip, same-identity persist idempotency, corrupt-lease fail-closed. (Those remediations predate the merge; the `MERGE_AUTHORIZATION = NOT_GRANTED` stamp that stood here applied to PR #654 before it merged and is historical.)
