...
- [x] AS-CODER-ALPHA-LIFECYCLE-READ-001 vault-scoped lifecycle-certify REPORT READ (`atlas lifecycle report` / `GET /v1/lifecycle` / `atlas.lifecycle.read`; LIFECYCLE!=AUTHORITY; CERTIFY_REPORT!=PILOT PASS; MISSING!=CERTIFIED; EMPTY!=HEALTHY; WRITE_APPLIED=false)
