...
- [x] AS-CODER-ALPHA-STATE-ATTENTION-WEB-MCP-001 first-class `/state` + `/attention` pages and vault-scoped `atlas.project-state.read` / `atlas.project-attention.read` MCP (uses existing LIVE_API; UI≠canonical; MCP≠authority)
