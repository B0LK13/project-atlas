...
- [ ] CTRL-YAML-CONSTRUCTOR-KEYERROR Map PyYAML ``!!bool`` constructor ``KeyError`` onto fail-closed control-plane refusals in preflight, readiness, and skill_loader (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). Does not authorize readiness. Does not touch Core ``ingestion.py``. Evidence: ``docs/evidence/CTRL-YAML-CONSTRUCTOR-KEYERROR.md``.
