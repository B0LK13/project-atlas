...
- [ ] CTRL-JSON-DECODE-FAIL-CLOSED Map ``json.JSONDecodeError`` onto fail-closed control-plane refusals in vault identity, authority grants, sessions, and repository-gate receipts (IMPLEMENTED; IV of ``3427d76c`` found VALID P1 JSON-null fail-open in ``repository_gate.validate`` -- remediating; merge NOT pre-authorized). Does not authorize grants. Does not touch Core ``ingestion.py``. Evidence: ``docs/evidence/CTRL-JSON-DECODE-FAIL-CLOSED.md``.
