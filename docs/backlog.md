...
_Status: **IMPLEMENTED — READY FOR OWNER MERGE GATE** (ORCH001D-011 independently verified 2026-08-28 across two rounds -- round 1 found and remediated a real P2 (subprocess capture memory-bound) and, on independent re-verification, a real P1 (fix silently defeated the timeout when stdin was present); round 2 independently re-verified the fix clean with 7 further adversarial variants; see WORKLOG "ORCH001D-011"; ORCH001D-012 authentic-Cursor acceptance remains separately outstanding, `ATTEMPTED_BLOCKED_ON_WORKSPACE_TRUST` (2026-08-29, owner-authorized genuine attempt, see D-206: two real `cursor-agent` subprocess dispatches made via the actual transport code, both correctly ask-mode/no-force; the vendor CLI itself refused to run non-interactively against a never-before-seen directory without `--trust`/`--force`/`-f`/`--yolo`, all of which `agent_transport.FORBIDDEN_CURSOR_FLAGS` correctly declines to ever pass; closing this requires one interactive trust-grant for a chosen throwaway directory, outside what a non-interactive pass can do) -- corrected 2026-08-28 from a false `EXTERNAL_BLOCKED` claim, see WORKLOG "Cursor CLI availability correction": a live Cursor CLI is genuinely present on this host; not merged; `MERGE_AUTHORIZATION = GRANTED` (2026-08-28, see docs/evidence/D-202-OWNER-AUTHORIZED-ORCHESTRATION-ACTIVATION.md)). Starts exactly one target agent for a governed `HANDOFF_READY` dispatchable task, then stops. Does **not** auto-dispatch the next hop. Does **not** resurrect PR #396. Does **not** start AS-ORCH-001E._
...
- [ ] ORCH001D-012 Authentic Local Windows Cursor agent dispatch acceptance (unaffected by activation below -- separately outstanding; genuinely attempted 2026-08-29, blocked on vendor CLI workspace-trust consent gate, not an Atlas defect -- see D-206)
...
- [ ] ORCH001E-011 `governor-loop-tick` CLI command has no node discovery/rehydration step -- constructs a brand-new empty `AutonomousGovernor` on every invocation (2026-08-28, found during independent IV of PR #637, independently reproduced; see docs/evidence/D-204-GOVERNOR-LOOP-TICK-NO-NODE-REHYDRATION.md). Cannot originate new work or recover interrupted work across separate process invocations -- only functions within a single long-lived process, same as every existing test/pilot-flow caller. Not fixed by PR #635 or PR #637 (both correct within their own tested scope; recovery fails earlier, on the empty governor's node lookup, before either fix's code is reached). Real fix requires a genuine node-rehydration design decision, not a narrow patch -- scoped as its own follow-up.
