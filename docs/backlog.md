...
- [ ] AS-OBSIDIAN-CAPTURE-001-F2 HUMAN region nesting semantics (OWNER DECISION REQUIRED). Nested regions with *distinct* names merge to a document carrying the inner block twice, and `a{x}` beside `b{x}` loses the first `x` outright: name-keyed resolution is last-wins, and the merge then splices the survivor into the earlier container. This is **live human-content loss**, not a hypothetical -- it predates F1 and F1 deliberately preserves it byte-for-byte (`test_the_same_name_in_two_different_scopes_is_not_an_f1_refusal` pins the equivalence) because deciding it was explicitly out of F1's scope. Decide whether nesting is supported, prohibited, or scoped; `backup.py`'s stack validator currently accepts ordinary nesting, so it is not forbidden today.
...
- [ ] AS-OBSIDIAN-CAPTURE-001-F4 Consolidate the third protected-region implementation (OWNER/POLICY GATED). `graph_projections._merge_protected_regions` is an independent copy that does **not** delegate to `protected_regions.py`, so F1's ambiguity fix does not reach graph projections. A 12-case differential put the two at 9 equivalent / 3 divergent -- graph preserves text outside the generated span when a note has no HUMAN regions, where the shared helper returns the fresh render. Consolidation must not be a blind replacement; characterise the divergences first, and do not claim a single implementation until it is literally true.
