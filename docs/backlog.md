...
- [ ] AS-OBSIDIAN-CAPTURE-001-F13 F5's sealed suite list names files that do not exist (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). The F5 WORKLOG abbreviation expanded `_adversarial` and `_r1_001` to stems that have no file. Corrected in place (not insertion-only -- disclosed). Eight real `tests/unit/*.py` paths written out; a sweep asserts every such path named in `WORKLOG.md` and `docs/evidence/*.md` exists. Issue #755's ninth path (F6) was not in the original abbreviation and is not inserted into the sealed paragraph.
