...
- [ ] ORCH001E-012 `loop.py::_complete_validated()` dangling `active_dispatch_id`: if a crash occurs between `apply_observed_result()` persisting `phase=VALIDATING` and its final `_save(...)` clearing `active_dispatch_id`, a resumed process's `_complete_validated()` sees `active_dispatch_id is not None` and silently no-ops forever (`return self._result()`) -- the loop is permanently stuck in `VALIDATING`, never crashing, never failing closed, never progressing. Found while implementing ORCH001E-011 (D-205 "found, out of scope"); D-205 stated this item would be recorded here but it was not until this correction (2026-08-29, found again during the D-206/persistent-autonomy native-rehearsal pass, independently re-checked absent from this file before adding it). Deliberately not fixed: `rehydrate_governor()` fails closed for `VALIDATING` as defense-in-depth on the *node*-lookup path, but this specific bug never reaches `governor.snapshot().nodes` at all, so that fail-closed check does not by itself prevent or cure the stuck-loop symptom. The correct terminal behavior (fail closed with a new code? retry `apply_observed_result` from persisted evidence, if any exists? something else?) is a genuine semantic design decision, not a narrow patch -- separately scoped, same as `ORCH001D-012`'s discovery gap.
