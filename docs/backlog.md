...
- [ ] AS-OBSIDIAN-CAPTURE-001-F4 Consolidate the third protected-region implementation (IMPLEMENTATION AUTHORIZED 2026-09-07; branch `fix/graph-projections-canonical-human-region-semantics` delegates HUMAN-region semantics to the canonical core via a narrow adapter, retaining one disclosed no-HUMAN outside-span contract; at Owner/IV gate -- merge NOT pre-authorized; see WORKLOG "F2 seal and F4 baseline"). `graph_projections._merge_protected_regions` is an independent copy that does **not** delegate to `protected_regions.py`, so F1's ambiguity fix does not reach graph projections. A 12-case differential put the two at 9 equivalent / 3 divergent -- graph preserves text outside the generated span when a note has no HUMAN regions, where the shared helper returns the fresh render. Consolidation must not be a blind replacement; characterise the divergences first, and do not claim a single implementation until it is literally true.
