...
- [x] AS-CODER-ALPHA-OPS-EVENTS-READ-001 vault-scoped `atlas ops-events` + `/v1/ops/events` + `#/ops-events` + `atlas.ops.events.read` (read-only; EMPTY!=HEALTHY; events≠authority; draft until owner merge)
