...
- [ ] YAML-CONSTRUCTOR-KEYERROR-CONTAINMENT Map PyYAML ``!!bool`` constructor ``KeyError`` onto structured refusals in ``load_safe_yaml``, estate marker parse, and graph-acceptance metadata (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). Sibling of F7-R1 / #819 (Obsidian ownership probe only). Does not touch ``ingestion.py``. Evidence: ``docs/evidence/YAML-CONSTRUCTOR-KEYERROR-CONTAINMENT.md``.
