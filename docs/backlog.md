...
- [x] AS-CODER-ALPHA-INCREMENTAL-CONNECT-001 no-change reconnect must not double discover+ingest (implemented on main: `project_atlas.incremental_connect` + `tests/unit/test_as_coder_alpha_incremental_connect_001.py`; skip is operational only, not Truth Core; MERGE_AUTHORIZATION NOT_GRANTED)
