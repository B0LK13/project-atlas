...
- [ ] AS-OBSIDIAN-CAPTURE-001-F11 The mkdir failure site in each writer (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). F6 closed the error boundary for the read of the prior note and for `os.replace`; creating the note's parent directory was left OUTSIDE the guard in **both** generated-span-preserving writers, so a blocked or unwritable parent escaped as a raw `OSError` past `ObsidianProjectionError` / `GraphProjectionError` -- the defect F6 exists to prevent, one step earlier in the same function. Reproduced on `a7adce4e` and again on current `main` by a plain file where a path component must be a directory: both writers leak `NotADirectoryError`. **Both sites were already recorded and an earlier revision claimed otherwise; that is retracted** -- F6's residual register names the `_promote` one explicitly ("an ancestor directory replaced by a file") 13 lines above the bullet this package quoted. The fix is new; the finding was not. Six lines of code replace one in each writer (`+11/-1` and `+10/-1` with comments), raising the module's own error type and naming the **directory**: `unwritable-note-directory:<Type>:<path>`. **The P0 this package paid for**: two tests asserted the literal string `"NotADirectoryError"`; Windows raises `FileExistsError` for the same fixture and CI went red, and on Linux alone the class depends on shape (ancestor file -> ENOTDIR 20, immediate parent -> EEXIST 17), so the assertion was platform- AND shape-coupled from the start. Fixed by **measuring**: a helper performs the same `mkdir`, catches the `OSError`, and the test asserts the guard names THAT class -- portable and strictly stronger, since a guard reporting a generic `OSError` now fails where the hard-coded string could not detect it. Under a plugin simulating the Windows class: new assertion 5 passed, old 2 failed / 3 passed. Windows CI green at `83a3d7a3` and again at `56c986f1`, 5,682 passed at each -- the SHAs are named because a backlog line outlives the PR. **A justification that was exactly backwards**: the docstring claimed a `pytest.raises` test "would pass on a writer that raised nothing at all" and used that to prefer a `try/except OSError`. `pytest.raises` fails with DID NOT RAISE and re-raises non-matching exceptions, so it rejects both failure modes; demonstrated with a mutant that swallows the `mkdir` failure and raises nothing -- old form 1 passed, new form 1 failed. That test was also redundant with the two above it and now runs a second, materially different blocked shape. **Controls** (each mutation under a sha256 assertion, sources restored byte-identical): baseline 5 passed; projection guard removed 3 failed; graph guard removed 3 failed; both guards swallow and raise nothing 4 failed / 1 passed; guard reports a generic `OSError` 3 failed; fixture made inert 3 failed (one fixture) or 4 failed (both); restored 5 passed. Read the 3/3 rows honestly -- each is carried by ONE guard-specific test, the other two being shared; judged on name sets neither contains the other, so both guards are independently load-bearing with one witness each, not three. **Reachability, corrected downward**: an earlier revision said the failure "surfaces today as an unhandled traceback"; not true at either production surface -- `cli.py:4264` catches `(ObsidianProjectionError, OSError, ValueError)` and `connect.py:786` catches `(OSError, ValueError, KeyError, TypeError)`, both already containing the raw error on base. Only `demo_readiness.py:162`, an internal harness, is unguarded. Weaker still on the graph side: `write_projection_outputs` has **no caller anywhere in `src/`** (seven test modules only). What this buys is precision and type-correctness at the boundary, not traceback-vs-diagnostic. **Not a policy change**, verified independently rather than argued: 67 legitimate scenarios at the previous head (attributed, not cited -- that harness is not in this repository) and 57 re-instrumented at this one with tree-level manifests compared by path and sha256, zero differing scenarios; plus 960 concurrent writes into a shared not-yet-existing tree with zero errors measured on base AND head. **What it does NOT close**: F6's register named THREE raw `_promote` escapes and this closes one; the other two remain raw (`graph_projections.py:620` read-only output directory, `:616` unreadable existing target) plus an unregistered `ENAMETOOLONG` at `:614`, all filed as **#757**. **The object matters more than the numbers**: the same three sites are at 603/605/609 on `ef628223`, 605/607/611 on current `main` (`e4dd17bc`), and 614/616/620 here. An earlier revision cited the first set and called it `main`; a first correction cited this set and called it the merged base -- both wrong about the OBJECT rather than the arithmetic, which is why the code expressions are the durable anchors. Issue #757 names `main` and therefore needs 605/607/611, not these; its body was corrected to those numbers on 2026-09-09, verified by read-back rather than by exit code. **Not claimed**: that every `OSError` in these modules is contained (the mkdir site only); that the F6 property holds "at all three sites" as an earlier docstring said; that `ingestion.py` is covered (a third writer, owner-gated behind a frozen surface needing an owner-approved sha256-pinned exception under `docs/atlas-3/ARCHITECTURE.md` §9.1); or that anything is verified on Windows beyond CI. Evidence: `docs/evidence/AS-OBSIDIAN-CAPTURE-001-F11-MKDIR-BOUNDARY.md`.
- [ ] AS-OBSIDIAN-CAPTURE-001-F12 The committed instruments must actually run (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). Three evidence instruments under `docs/scripts/` were each committed because "an instrument described is not an instrument available", and measured on `a7adce4e` **none of the three is referenced by any test or CI file**: `pyproject.toml` sets `testpaths = ["tests"]` so pytest never collects them, and ruff's `include` covers only `src/**/*.py` and `tests/**/*.py`, so they are not linted either -- they can be renamed or broken by a refactor with nothing noticing, which is the same failure the seals were meant to close, one level up. Scope differs per instrument: `f9_diagnostic_parity.py` has a decidable contract and is **executed** with its result asserted; `f8_near_miss_controls.py` rewrites source files and restores them in a `finally`, so it is import- and interface-checked only; `seal_retracted_claim_sweep.py` over-reports by design and needs a human to classify hits. **Five revisions of one guard, four defeated by verification on the round after each was called complete**: a source substring (needle contained a space, haystack had them stripped -- it could never match, and the subprocess path that would really run the mutating instrument passed UNDETECTED); AST call sites (15 of 20 paths slipped past, including `_load(_MUTATING).main()` using the module's own constant, on the very line the guard test writes); a runtime fuse (which published its own bypass -- the real callable on `__wrapped__`, the standard unwrapping convention, plus a second unfused loader added by the same commit); the content hash (a same-named file with ONE BYTE edited loaded unfused, and a test of mine asserted that behaviour, so the suite was DEFENDING the gap while its control passed); and finally **filename OR content hash**, the union rather than a sixth replacement. **The lesson generalises**: replacing the identity a guard keys on trades one hole for another, because neither rule is a superset of the other -- other-name/identical-bytes escapes the name rule and is blocked by the hash; same-name/one-byte-edited escapes the hash rule and is blocked by the name. Each replacement felt like a strict improvement because it closed the case the reviewer had just named, and nobody re-measures the column that used to pass. Both halves are load-bearing (removing either fails a test), and the predicate has not collapsed to `True`: unrelated code under a different name is still not fused, and `f9`, the one instrument that genuinely executes, still runs to exit 0. The second layer is a static detector for primitives that bypass the loader (`subprocess`, `runpy`, `os`, `importlib`, `asyncio`, `exec`/`eval`/`compile`), resolved through import aliases, module-name spellings and any name EVER bound to something naming the script, deliberately order- and scope-blind. It **fails closed**: an earlier revision capped the name-resolution fixpoint at 16 and returned what it had, so a long enough reverse-ordered chain produced an empty -- i.e. "clean" -- verdict with no error. **Two residues, measured and not closed**: a copy changing BOTH name and content matches neither half; and the detector does not reach a path that both bypasses the loader and names the script through no resolvable constant. Nothing plausible produces either by accident, which is a different claim from saying they are absent. **A stale `__pycache__` hazard** found by verification and reproduced: a same-size, same-mtime edit leaves the pyc header matching, so the loaded module executes OLD code while the file says something else -- inert in CI, live in the local edit-test loop where this lane runs its negative controls, and where a control has already been misled by a stale snapshot once. The loader now drops cached bytecode first, pinned against a throwaway file with the hazard reproduced inside the same test. **Sixteen controls**, each under an anchor assertion with the file restored byte-identical, baseline 19 passed; **six of those rows passed at the revision before the one that added them**. **Claims corrected under verification**: "one pass could not see it" was wrong (a single pass resolves forward chains; only reverse-ordered ones need iteration); "14 of 14 detected" had no derivable denominator (the corpus is 22, all 22 detect); and the `CITED` comment claimed "only `main` appears at all" when four of ten attributes appear zero times -- now stated WITH its method, since the prior round could not reproduce the bare figure and the missing method was exactly why. **Not claimed**: that the instruments are *correct*, only that they still load and, for the one with a decidable contract, still report what the record says; and `f9_diagnostic_parity.py` always passes `rendered=FRESH`, so it varies only the prior note and **cannot observe a rendered-side divergence at all** -- exactly the class F10 needed a two-sided sweep to find. **A gap recorded rather than fixed**: `seal_retracted_claim_sweep.py` is cited by NO evidence record, backlog line or WORKLOG line anywhere -- committed as reproducible evidence with nothing referring to it, the same rot this package catches, one level further out.
- [ ] AS-OBSIDIAN-CAPTURE-001-F16 Human bytes across writes that SUCCEED (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). Every protected-region test in this repository pins a **refusal**; the dangerous case is the opposite one -- a write that SUCCEEDS while altering operator bytes, because by construction every marker, count and placement check has already passed on it. Measured on `b87b4a22` by parsing the test tree rather than reading it: **8** assertions across two modules touch a region VALUE (two more touch only the key set), and **0** compare an extraction against another extraction -- so the invariant was asserted nowhere AS A BYTE-EXACT INVARIANT. Of the 8, only 2 compare against a true literal. `test_f4_differential_randomized_matches_canonical_core` already runs a 500-trial randomized success-path sweep at base, checking substring containment on well-behaved ASCII -- so the byte-exact, whitespace and CRLF gap is real, but "asserted nowhere" was too broad. Both qualifications came from independent verification. **A hypothesis discarded before writing anything**: I expected the existing `.strip()`-based assertions to be blind to whitespace and newline changes; measured, they are not, because `.strip()` touches only the region's outer edges while the extracted region includes its BEGIN/END markers. The existing assertions are byte-capable; what was missing is the invariant, a corpus, and the success path to disk -- a narrower claim than the one #760 was filed with, and #760 has been corrected to it. Adds a corpus of 204 prior/rendered pairs (tabs, leading/trailing runs, CRLF, blank lines, no-break space, zero-width space, emoji, non-ASCII, backslashes, marker-shaped text, a 300-character body, and two-region notes whose render lists them in the OPPOSITE order) swept through every generated-span-preserving merge reachable without an owner-gated exception, then followed to disk through all three atomic writers and re-extracted: **accepted 366, refused 14, human bytes changed 0**. The 14 refusals matter as much as the 366 -- they are evidence the corpus still reaches the accept/refuse boundary, so `MIN_REFUSED` is asserted. **The counts moved from 332/12** when verification found an inert name column: an earlier revision included `"n o t e s"` and the marker grammar is `[^\s>]+`, so twenty advertised pairs produced no region at all while still counting toward the corpus size; a test now asserts every NAME parses. **The zero is falsifiable in the same test run**: **sixteen controls**, each under an anchor assertion with the file restored byte-identical (`3f2f4cba`) -- baseline 8 passed, and every mutation that alters human bytes in a swept path fails a named test, including per-writer isolation (corrupting only `capture_io`, only `obsidian_projection._write_atomic`, or only `graph_projections._promote` each fails the round trip). **Two mutations initially reported green because their anchor did not match** -- shell escaping mangled a CRLF literal, and a tuple element had no leading whitespace -- and an inert mutation reporting green is the same defect this module exists to catch, so both were re-applied through a file-based mutator before being counted. **Two controls were rewritten because they could not fail** -- they removed an assertion, and deleting an assertion can never fail a test; the honest form is to break what the guard PROTECTS. **The alias assertion was a third**: discovery filters on the un-aliased name, so alias resolution was never load-bearing for it and the assertion passed just as readily on a derivation resolving no aliases at all; it is replaced by feeding the detector synthetic renamed and re-exported modules. **What the derivation does NOT close**, measured across two verification rounds: a second round found a FIFTH evasion class in three idiomatic spellings -- a splicer reached as an attribute in non-call position (`_F = pr.merge_...` then `_F(...)`, `functools.partial`, `staticmethod`) -- not route-arounds but what someone writes without thinking, now closed by an `ast.Attribute` clause measured free, alongside widening the scan root from one package to all of `src/` since `src/atlas_contracts/` is a real shipped package. Three evasions survive: importlib+getattr with split literals, `__dict__` lookup, and a hand-rolled splice via `Path.read_bytes` -- the last falsifying the premise that those three functions are the only way to reach a prior note's human bytes. Two further limits are pinned rather than argued: a two-hop re-export under an unlisted name evades, and the alias list ROTS if a writer renames its local alias, so a test checks the list against the tree in both directions. It makes the ACCIDENTAL new writer hard to introduce; it does not make the set closed. Adding `ingestion` fails the INVARIANT test as well as the exclusion test, demonstrating the #759 CRLF defect through this harness, so the exclusion protects against a reproduced defect rather than a hypothetical one. **Owner-gated, not fixed**: `ingestion._generated_content` is the fourth writer, excluded because `ingestion.py` is on the frozen surface enumerated by `test_atlas3_demo_isolation_001`; fixing it needs an owner-approved sha256-pinned exception under `docs/atlas-3/ARCHITECTURE.md` §9.1 which this lane cannot self-grant, and the exclusion is asserted by a test naming the condition for reversing it. **Not claimed**: that the writers are correct in general (only that across this finite hand-chosen corpus no accepted write altered human bytes); that `HUMAN_CONTENT_INTEGRITY = CONTINUOUSLY_VERIFIED` holds for Atlas as a whole -- three of four generated-span-preserving writers are now continuously verified on the success path, the fourth is #759 and remains owner-gated, so the honest statement is *continuously verified for the writers this lane may touch, with the exact blocker identified*; that refusal coverage changed; or that any `src/` file is modified -- none is. **The writer set was wrong and is now enforced rather than declared**: an earlier revision swept two merge paths and two atomic writers and called that the full set; derived mechanically from the source tree there are FOUR modules that can splice human regions (`graph_projections`, `obsidian_projection`, `obsidian_capture_note`, and the frozen `ingestion`). `obsidian_capture_note` was the miss -- it shares the canonical merge but has its OWN atomic writer delegating to `capture_io.write_atomic_under_root`, so the previous disk round trip claimed coverage broader than the code behind it; it is now swept, and corrupting that path to normalise CRLF fails the round-trip test. The set is enforced by derivation at test time, so a module that can splice and is neither swept nor explicitly excluded fails -- measured by dropping a hypothetical fifth splicing module into `src/`. The derivation follows ALIASED imports, which matters because all three non-frozen writers rename the canonical merge on import and a name-only match would have missed every one of them. **The guard caught its author on its first run**, rejecting a coverage entry for `protected_regions.py`, which defines the merge rather than splicing into a prior note. Evidence: `docs/evidence/AS-OBSIDIAN-CAPTURE-001-F16-ACCEPTED-WRITE-INTEGRITY.md`.
- [ ] AS-OBSIDIAN-CAPTURE-001-F18/F19 Runtime write boundary and write attribution (IMPLEMENTED; AWAITING IV -- merge NOT pre-authorized). Two questions kept apart because they have different answers and different limits: F18 asks whether a write altered operator-owned bytes, F19 asks which execution did it, under what authority, against which exact state. **F18** intercepts the OS primitives note bytes reach disk through (`os.replace`, `os.rename`, `os.open`, `os.close`, `os.fdopen`, `builtins.open`, `Path.write_bytes`, `Path.write_text`) rather than Atlas's own helpers, so it is spelling-independent -- a writer need not call the blessed helper to be seen. Twelve bypass routes enumerated, twelve caught; and the detector is itself falsifiable, because unhooking each primitive in turn makes the corresponding route escape, without which "all routes caught" would be equally consistent with a detector that always reports success. **F18 detects damage but cannot attribute it, and an earlier reading of it was wrong**: `Violation.origin` names the innermost frame in `src/`, which distinguishes PLUMBING, not responsibility -- a caller handing damaged bytes to `capture_io.write_atomic_under_root` and a caller writing them directly are the same act -- so it is renamed `landed_via_atlas_helper`, must never be used as a gate, and the limitation is pinned by a test. **The boundary is opt-in, a measured retreat rather than a preference**: session-wide installation broke three tests that spawn a nested `pytest` (the child loads the same conftest and the added option changed its exit status), and a monitor that changes the result of the suite it observes is not an observer; making it continuous requires that interaction fixed first, which belongs to the owners of those tests and is recorded rather than worked around. One opt-in full-suite run reports `568 protected writes checked, 82 altered operator regions` -- those 82 are NOT 82 defects, most being fixtures legitimately rewriting their own notes, and presenting the raw count as a defect count would be the overclaim this entry exists to avoid. **F19 introduces no new identity system**: `AgentLease` already carries `agent_id`/`lease_id` (which execution), `capabilities`/`authorized_paths`/`forbidden_paths`/`active` (under what authority) and `base_pin` (against which exact state); the record was well formed, what did not exist was any CONNECTION between it and a write, and that connection -- a `ContextVar` binding trusted via a MINTED TOKEN rather than via the presence of a lease object -- is the whole of F19. Seven mutations each turn the suite red (trust any claimed binding: 2 failed; authorize everything: 4 failed; ignore forbidden paths / ignore `active=False` / let `reconcile` claim observation it lacked / never destroy the token on exit: 1 failed each; treat silence as clean: 2 failed), baseline and restored 14 passed. Inheritance across concurrency is measured, not assumed: an awaited task stays attributed, a bare thread starts with an empty context and is reported unattributed. A vocabulary test that asserted a hand-written set against the enum was replaced because it could not fail for the reason it claimed -- the literal would have been updated alongside any new member and stayed green while nothing emitted it -- so every state is now produced by a real call. **Demonstrated, not deployed**: nothing in `src/project_atlas` calls `bind()`, so F19 proves the binding works without making Atlas attribute its own writes today; the integration point is named rather than left vague, since `leases.py:grant_lease` is the sole place an `AgentLease` is ever created and is reached in production from `governor.py:596`, so ONE `bind()` there would cover every governed execution -- a `src/` change this lane has not made, recorded as a handoff. **Not claimed**: `A_WRITER_CANNOT_SILENTLY_BYPASS_HUMAN_CONTENT_INTEGRITY` (report-only in-process, absent out-of-process); cross-process attribution (the process boundary carries no trusted identity, so a subprocess write is `OUTSIDE_OBSERVABLE_BOUNDARY` and stays there); a production control (both modules live under `tests/`, nothing in `src/project_atlas` imports them, no shipped path changes); a defence against hostile in-process code, which can mint itself a token exactly as it can unhook the boundary -- pinned by a test that PASSES WHEN THE ATTACK SUCCEEDS; or closure of #759, whose frozen surface is untouched. Evidence: `docs/evidence/AS-OBSIDIAN-CAPTURE-001-F18-F19-WRITE-BOUNDARY-AND-ATTRIBUTION.md`.
