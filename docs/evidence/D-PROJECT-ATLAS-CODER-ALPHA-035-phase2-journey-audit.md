...
| 2 | `atlas connect .` | **MISSING** | — | No `connect` subcommand in `atlas --help` / `src/project_atlas/cli.py` (confirmed absent). Closest fragments: `init` + `discover`/`ingest` + Windows start scripts + `atlas_agent init-project` — none named or behaving as `connect` |
