...
CONTROLS = {
 ...
 "NC-N locator collision check dropped": ("src/project_atlas/atlas3/contracts.py",
 ...
 "NC-U symlink component walk dropped": ("src/project_atlas/atlas3/contracts.py",
   "        if current.is_symlink() or current.is_junction():", "        if False:"),
 "NC-V resolved containment re-check dropped": ("src/project_atlas/atlas3/contracts.py",
   "    if not target.resolve().is_relative_to((root / namespace).resolve()):", "    if False:"),
 ...
 "NC-AB task-dir-is-file check dropped": ("src/project_atlas/atlas3/contracts.py",
   "    if parent.exists() and not parent.is_dir():", "    if False:"),
}
