...
TARGET_HEAD = "1f04c63bdc0ec503a6fd36c25172d564d06eed3a"
TARGET_TREE = "77c3df260923f29362cd742d6d313f46653b9170"
RECEIPT_DIR_REL = Path(".atlas") / "orchestration" / "sdk-runtime"
HARBOR_FIXTURE_REL = Path("tests/fixtures/demo/estate/harbor-api")
DEMO_ESTATE_REL = Path("tests/fixtures/demo/estate")
...
def lane_exact_main(repo: Path) -> dict[str, Any]:
    head = _git_sha(repo)
    ...
    return {
        ...
        "TARGET_TREE_MATCH": tree == TARGET_TREE,
        "PR438_FIX_PRESENT_IN_MAIN": pr438_merge.returncode == 0,
        "PR431_IN_MAIN": subprocess.run(
            ["git", "merge-base", "--is-ancestor", "72b6d255aa6a6a7d987cdc59f75657c0d4122136", head],
            cwd=repo,
            capture_output=True,
        ).returncode == 0,
        "PR433_IN_MAIN": head == TARGET_HEAD or subprocess.run(
            ["git", "merge-base", "--is-ancestor", "d5e1e988c090fc3fc783ef9913f8043c4efb22b4", head],
            cwd=repo,
            capture_output=True,
        ).returncode == 0,
    }
...
def lane_liveness(
    ...
    resident_pid = detach_resident_driver(root=root, package_src=package_src, python=sys.executable)
    for _ in range(30):
    ...
    watchdog_alive = pid_is_alive(read_watchdog_pid(root))
    dup_in_window = max(s["duplicate_dispatch_count"] for s in samples)
    return {
...
def lane_authentic_pilot(repo: Path, work: Path, env: dict[str, str]) -> dict[str, Any]:
    ...
    if ask_proc.returncode == 0:
        try:
            ...
            ask_ok = ask_payload.get("status") == "known" and ask_payload.get("evidence_count", 0) > 0
    ...
    return {
        ...
        "PILOT_CONFIG": {"fixture_only": False, "synthetic_shortcut": False, "production_path": True},
        "FIXTURE_ONLY": False,
...
def lane_clean_machine(target_head: str, python_for_venv: str) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="atlas-d144-clean-") as temp_name:
        ...
        clone = scratch / "clone"
        subprocess.run(
            ["git", "clone", "--depth", "1", "https://github.com/B0LK13/project-atlas.git", str(clone)],
            check=True,
            capture_output=True,
        )
        subprocess.run(
        ...
        env = os.environ.copy()
        version = subprocess.run([str(py), "-m", "project_atlas.cli", "version"], cwd=clone, capture_output=True)
        core = subprocess.run(
        ...
        adv = subprocess.run(
            [str(py), "-m", "project_atlas.cli", "adv", "certify", "--work-root", str(adv_work), "--json"],
            cwd=clone,
            capture_output=True,
            text=True,
        )
        ...
        if adv.returncode == 0:
            try:
                ...
                adv_pass = False
        restart = subprocess.run([str(py), "-m", "project_atlas.cli", "version"], cwd=clone, capture_output=True)
        return {
            ...
            "INSTALL": core.returncode == 0 or golden.returncode == 0,
...
def lane_integrated_iv(repo: Path, env: dict[str, str]) -> dict[str, Any]:
    ...
    ruff = subprocess.run(
        [sys.executable, "-m", "ruff", "check", "src/project_atlas/orchestration/sdk/ci_observer.py"],
        cwd=repo,
        capture_output=True,
        text=True,
    )
...
def lane_main_ci(repo: Path) -> dict[str, Any]:
    obs = observe_exact_head_ci(head_sha=TARGET_HEAD)
    return {
...
def aggregate(
    *,
    ...
    o1_closed = bool(liveness and liveness.get("O1_RESIDENT_PASS") and liveness.get("O1_WATCHDOG_PASS"))
    o2_closed = bool(pilot and pilot.get("AUTHENTIC_PILOT"))
...
def main_cli() -> int:
    ...
    main_lane = lane_exact_main(repo)
