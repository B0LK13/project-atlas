...
- Live bridge `/v1/mission-journey` returned schema-valid data for `B0LK13/project-atlas` from the live read path; `/v1/task-context` returns a bounded, schema-valid packet immediately; unavailable frontier and stack enrichment is listed as `NO_FRONTIER_MATRIX`/`NO_STACKS`.
