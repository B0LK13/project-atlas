...
| A2 | Governed claim/dispatch/handoff/open-worktree/terminal | **Scope READY / implementation NOT_STARTED** — see `a2/` + ADR-035. First WP `AS-STUDIO-A2-001` = governed OWNERSHIP_CLAIM intent (not BUTTON→MUTATION). Exit evidence when implemented: denial/authz/freshness/idempotency tests; control-plane execute/refuse; audit/receipt. |
