...
|---|---|---|
| A0 | Repository reuse map; daemon/Studio ADR; API/events, permissions and plugin contracts; Linux shell spike | Exact commit/tree inventory; contract schemas and compatibility policy; threat model; core reuse decisions; narrow A1 backlog; credible Linux build evidence |
| A1 | Read-only Mission Control: DAG/frontier/agents/ownership/CI/IV/stack/seal | Real daemon data; no CLI text parsing; no mutation endpoints exposed; source/evidence links; stale/unknown/offline states; reconnect and UI-crash checks |
