# Atlas 2.1 — Package board

Tip: `14ae2e2` + ADV live harden. `ATLAS_2_1_RELEASE_CERTIFIED = NO`.

| Package | Wave | Status | Notes |
|---|---|---|---|
| AS-2.1-DOC-REALITY-001 | 1 | MERGED #143 | Audit + matrix |
| AS-2.1-APP-SVC-001 | 1 | MERGED #143 | App service |
| AS-2.1-API-SERVER-001 | 1/2/3 | MERGED + deepen | CORS/cap/ledger/MCP list |
| AS-2.1-WEB-LIVE-001 | 1 | MERGED #143 | LIVE_API prefer |
| AS-2.1-MCP-SERVER-001 | 1/3 | MERGED + deepen | list_mcp_tools |
| AS-2.1-OAI-IMPORT-REAL-001 | 1 | MERGED #143 | Real export |
| AS-2.1-SCHED-LIVE-001 | 1 | MERGED #143 | Supervised |
| AS-2.1-AUTHZ-001 | 1/2/3 | MERGED + deepen | + oai.responses + audit |
| AS-2.1-PILOT-AUTH-001-PREP | 1/3 | MERGED + expand | Bounded env/workspace scan |
| AS-2.1-AUTONOMY-L3-001 | 2 | MERGED #144 | L3 bounded |
| AS-2.1-CHATGPT-BRIDGE-001 | 2 | MERGED #145 | Export bridge |
| AS-2.1-COLLAB-001 | 2 | MERGED #145 | Session receipts |
| AS-2.1-WEB-ACTIONS-001 | 2 | MERGED #145 | Action ledger |
| AS-2.1-PROV-LIVE-001 | 2 | MERGED #145 | local-model live |
| AS-2.1-ASK-ATLAS-LIVE / OBS-LIVE | 2 | MERGED #145 | Polish |
| AS-2.1-OAI-RESPONSES-POC-001 | 3 | MERGED #146 | EXPERIMENTAL non-blocking |
| AS-2.1-ADV-LIVE-001 | 3 | LANDING | ADV suite + OBS deepen |
| AS-2.1-PILOT-AUTH-001 | — | **OWNER_BLOCKED** | FOUND=0; wake AUTHENTIC_ESTATE_ROOT_AVAILABLE |
| AS-2.1-SYNC-AUTH / TWIN-AUTH | 3 | BLOCKED on PILOT | Authentic only |
| AS-REL-2.1-001 | RC | NOT OPEN | v2.1.0 |

Hardening continues; do **not** declare board empty. Release still gated on authentic PILOT PASS.
