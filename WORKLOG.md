...
**Date:** 2026-09-13
**Branch:** `fix/at3014-ledger-path-integrity`
**Base:** `b87b4a226f4aa8b2f669edf112aa3476454f754f`
**MERGE_AUTHORIZATION:** NOT_GRANTED

Independently reproduced on live main: a directory at
`generated/ops/atlas3/ledger/<project_id>.jsonl` made `list_events` return
`[]` (healthy empty). A planted symlink of valid harbor-api events was
consumed as the project ledger, and `append_event` wrote through it.
`append_event` on a directory leaked `IsADirectoryError`.

Fix: `_regular_ledger_file()` fail-closes `LEDGER_CORRUPT` when the ledger
identity is a symlink or a non-file. Missing stays empty. Append uses the
same check before write.

Does not touch `ingestion.py`. Does not write Truth Core. Does not
authorize merge. Does not widen the declared-graph `is_file()` sweep.

Evidence: `docs/evidence/AT3-014-F2-LEDGER-PATH-INTEGRITY.md`
