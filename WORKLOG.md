...
raw `rcap-*.json` with `"title":"\\u0041KIAAAAAAAAAAAAAAAAA"` has no ASCII
`AKIA`, so `scan_text` of the file is empty. `retry` / `_persist_record`
decode and rewrite the token under `generated/ops/raw-captures/`.

Remediation: `_scan_decoded_record` before persist and after retry load.
Distinct from #977 / #988 / listed P2 session_capture.
