...
**Date:** 2026-09-13
**Branch:** `fix/at3014-append-verify-before-write`
**Base:** `b87b4a226f4aa8b2f669edf112aa3476454f754f`
**MERGE_AUTHORIZATION:** NOT_GRANTED

Independently reproduced on live main: `append_event()` accepted a
caller-supplied event after only a `project_id` check. A tampered
`content_hash` (`sha256:` + 64 `f`s) was persisted as `idempotency=appended`.
Subsequent `list_events` / Pulse / Start then fail closed on the poisoned
row (`CONTENT_HASH_MISMATCH`), so a write-path hole becomes a read-path
outage.

Fix: after the existing `project_id` check and before `list_events` / write,
call `verify_engineering_event(record, expected_project_id=pid)`. Tampered
hashes raise `CONTENT_HASH_MISMATCH` and the ledger file is not written.

Does not touch `ingestion.py`. Does not write Truth Core. Does not authorize
merge.

Evidence: `docs/evidence/AT3-014-F1-APPEND-VERIFY.md`
