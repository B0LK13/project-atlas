...
raw session JSON `{"marker":"\\u0041KIAAAAAAAAAAAAAAAAA"}` has no ASCII
`AKIA`, so `scan_text` of the file is empty. `json.loads` (including
`spool_sync.synchronize`) decodes the escape and `session.save` rewrites
`.atlas/sessions/<id>.json` with the decoded token.

Remediation: rescan decoded scalars before `session.save` and fail closed.
Distinct from #988 (conversation-capture review rewrite) and listed P2
`session_capture`.
