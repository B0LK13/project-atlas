...
## AS-SEC-SCAN-ORCH-SDK-RUNTIME-PERSIST-JSON-ESC-001 — decoded SDK persist keys

Independent leftover hunt on live main
`b87b4a226f4aa8b2f669edf112aa3476454f754f` reproduced NFR-004 persist:
raw SDK runtime JSON with object keys / values `"\\u0041KI…"` has no ASCII
`AKIA`, so `scan_text` of the file is empty. Incremental writers then
dump decoded `AKIAAAAAAAAAAAAAAAAA`.
