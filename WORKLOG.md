...
raw grant JSON with `"issuer_id":"\\u0041KIAAAAAAAAAAAAAAAAA"` has no
ASCII `AKIA`, so `scan_text` of the file is empty. `revoke_grant` decodes
and rewrites the token under `<store>/grants/<id>.json`.

Remediation: rescan decoded grant scalars before rewrite and fail closed.
Distinct from #828 / #893 / #992.
