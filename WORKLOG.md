...
**Date:** 2026-09-13
**Branch:** `fix/at3014-payload-must-be-object`
**Base:** `b87b4a226f4aa8b2f669edf112aa3476454f754f`
**MERGE_AUTHORIZATION:** NOT_GRANTED

Self-consistent non-object ledger `payload` was accepted by `verify_engineering_event`, counted as healthy by `ledger_status`, and crashed Pulse with `AttributeError`. Verify now requires `payload` to be a dict (`LEDGER_SCHEMA_INVALID`). Empty `{}` remains valid. `ingestion.py` untouched.

Evidence: `docs/evidence/AT3-014-F2-PAYLOAD-MUST-BE-OBJECT.md`
