...
raw JSON `{"source":"\\u0041KIAAAAAAAAAAAAAAAAA"}` has no ASCII `AKIA`, so
`scan_text` of the file is empty. `json.loads` decodes the escape and
`write_quarantine_outputs` writes the decoded token under
`generated/graph/quarantine/<project>/gq-*.json`.

Remediation: `_safe_graphify_id` / `_safe_graphify_ids` before persist of
`source_graphify_id`, `target_graphify_id`, `graphify_edge_ids`, and
`source_candidate_id`. Distinct from #983 (relationship-quarantine only).
