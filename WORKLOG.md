...
## AS-SEC-SCAN-FEDERATION-VAULT-ROOT-JSON-ESC-001 — decoded vault roots must not persist

Independent leftover hunt on live main
`b87b4a226f4aa8b2f669edf112aa3476454f754f` reproduced NFR-004 persist:
JSON `{"vault_root":"\\u0041KIAAAAAAAAAAAAAAAAA"}` has no ASCII `AKIA`,
so `scan_text` of the raw bytes is empty. `json.loads` decodes the
escape and `build_join_inventory` writes the decoded token into a
refused join inventory under `generated/federation/`.

Remediation: scan decoded `vault_root` and `project_id` before persist.
Distinct from #989 (Atlas 3 memory persist).
