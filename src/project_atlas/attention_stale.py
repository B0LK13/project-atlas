"""AS-CODER-ALPHA-ATTENTION-STALE-001 — CLEAR must not hide live source drift.

Attention reads vault review/compile artifacts only. After a disk edit,
delete, or rename without reconnect, ``rollup=CLEAR`` can look current.
...
Independent of #380/#381/#383/#385. Does not rewrite ``connect.py`` or
``cli.py``.
"""
...
PACKAGE_ID: Final[str] = "AS-CODER-ALPHA-ATTENTION-STALE-001"
GENERATOR_ID: Final[str] = "atlas-coder-alpha-attention-stale-001"
...
class AttentionStaleError(ValueError):
    """Fail-closed attention stale-inventory error."""
...
def _safe_project_id(project_id: str) -> str:
    try:
        ...
        raise AttentionStaleError(str(exc)) from exc
