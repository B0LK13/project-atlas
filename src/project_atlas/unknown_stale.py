...
The unknown lens reads vault review/compile artifacts only. After a disk
edit, delete, or rename without reconnect, ``rollup=clear`` (or an
unchanged unknown summary) can look current.
...
Independent of #380/#381/#383/#385/#386. Does not rewrite ``connect.py``
or ``cli.py``.
"""
...
PACKAGE_ID: Final[str] = "AS-CODER-ALPHA-UNKNOWN-STALE-001"
GENERATOR_ID: Final[str] = "atlas-coder-alpha-unknown-stale-001"
...
class UnknownStaleError(ValueError):
    """Fail-closed unknown stale-inventory error."""
...
def _safe_project_id(project_id: str) -> str:
    try:
        ...
        raise UnknownStaleError(str(exc)) from exc
