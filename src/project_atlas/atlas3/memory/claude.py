...
Claude history API. CLAUDE.md is bootstrap, not ingestion.
...
def _turns_from_payload(payload: object) -> list[dict[str, Any]]:
    if isinstance(payload, dict):
        if payload.get("live_full_history_sync") is True:
            raise Atlas3Error(
                "CLAUDE_HISTORY_API_CLAIMED",
                "Claude fixture must not claim live_full_history_sync",
            )
        raw = payload.get("messages")
        if raw is None:
            raw = payload.get("turns")
        if not isinstance(raw, list):
            raise Atlas3Error("CLAUDE_EXPORT_INVALID", "messages/turns must be a list")
    elif isinstance(payload, list):
        raw = payload
    else:
        raise Atlas3Error("CLAUDE_EXPORT_INVALID", "export must be an object or list")
    turns: list[dict[str, Any]] = []
    for row in raw:
        if not isinstance(row, dict):
            raise Atlas3Error("CLAUDE_EXPORT_INVALID", "turn is not an object")
...
def import_claude_export(
    ...
    *,
    ...
    """Parse a Claude-style JSON export into canonical envelopes."""
    if isinstance(source, Path):
        if source.name == "CLAUDE.md":
            raise Atlas3Error(
                "CLAUDE_MD_IS_NOT_INGESTION",
                "CLAUDE.md is a bootstrap adapter, not an ingestion source",
            )
        ...
        try:
            ...
            raise Atlas3Error("CLAUDE_EXPORT_INVALID", "export is not readable JSON") from exc
    else:
        try:
