...
def persist_search(vault: Any, project_id: str, result: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(hits, list):
        raise Atlas3Error("SEARCH_INVALID", "search hits must be a list")
    for index, hit in enumerate(hits):
        if not isinstance(hit, dict):
            raise Atlas3Error("SEARCH_INVALID", f"hit[{index}] must be an object")
        hit_pid = hit.get("project_id")
        if hit_pid is None or str(hit_pid).strip() == "":
            raise Atlas3Error(
                "PROJECT_MISMATCH",
                f"hit[{index}] missing project_id under governed persist",
            )
        if str(hit_pid) != pid:
            raise Atlas3Error(
                "PROJECT_MISMATCH",
                f"hit[{index}] project_id {hit_pid!r} != requested {pid!r}",
            )
