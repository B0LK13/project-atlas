...
This is the single ``validate`` / ``issue`` implementation. Orchestration must
not copy these rules. Sibling ``agent_control.receipt_gate`` re-exports this
module.

SEC-016: a self-issued session receipt is evidence only — never authority.
"""
...
from project_atlas.agent_control import session
...
def validate(state: dict[str, Any]) -> list[str]:
    ...
    if state.get("pipeline", {}).get("pending_spool", 0) and state.get("preflight", {}).get(
        "strict", True
    ):
    ...
    if captured and not all(
        int(pipeline.get(key, 0)) >= captured for key in ("normalized", "verified", "routed")
    ):
...
def issue(root: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    rehearsal = state.get("skill", {}).get("id") == "atlas-governed-work" and str(
        state.get("session", {}).get("task_id", "")
    ).startswith("AS-SKILL-001")
    payload = {
        "schema_version": 1,
        "receipt_type": "atlas-governed-work-rehearsal" if rehearsal else "atlas-agent-session",
        "receipt_id": "ASR-"
        + hashlib.sha256(str(state["session"]["session_id"]).encode()).hexdigest()[:16],
        ...
        "agent": {
            **state["agent"],
            "adapter_readiness": state.get("preflight", {}).get("readiness"),
        },
        "skill": {
            **state["skill"],
            "hash_matched": True,
            "verified": True,
            "acknowledged": bool(state.get("skill_acknowledgement")),
            "certification_receipt": state.get("preflight", {}).get("skill_certification"),
        },
        "skill_acknowledgement": state.get("skill_acknowledgement"),
        "vault": state["vault"],
        "events": state["events"],
        "pipeline": {**state["pipeline"], "failed": 0},
        "capability": state.get("capability", {}),
        "validation": {
            "skill_certification": "passed",
        ...
        "rehearsal": {
            ...
            "adapter_id": state["agent"].get("adapter_id"),
