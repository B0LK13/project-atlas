        ...
        def do_GET(self) -> None:
            ...
            if path == "/v1/inbox":
                ...
                if not project:
                    self._send(
                        400,
                        {
                            "error": "inbox-requires-project",
                            "package_id": "AS-CODER-ALPHA-INBOX-API-001",
                            "honesty": "UNSUPPORTED_SCOPE",
                        },
                    )
                    return
                raw_status = (qs.get("status") or [""])[0].strip() or None
                try:
                ...
                try:
                    self._send(
                        200,
                        service.inbox(project, status=raw_status, limit=limit),
                    )
                except AppServiceError as exc:
                    honesty = getattr(exc, "honesty", None) or "MALFORMED_INPUT"
                    self._send(
            ...
            if path == "/v1/decisions":
                ...
                if not project:
                    self._send(
                        400,
                        {
                            "error": "decisions-requires-project",
                    ...
                    return
                try:
                    self._send(200, service.decisions(project))
                except AppServiceError as exc:
                    honesty = getattr(exc, "honesty", None) or "MALFORMED_INPUT"
                    self._send(
                        400,
            ...
            if path == "/v1/attention":
                ...
                if not project:
                    ...
                    return
                try:
                    self._send(200, service.attention(project))
                except AppServiceError as exc:
                    honesty = getattr(exc, "honesty", None) or "MALFORMED_INPUT"
                    self._send(
                        400,
