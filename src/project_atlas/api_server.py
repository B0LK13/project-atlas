    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if path == "/v1/knowledge":
                ...
                return
            if path == "/v1/reviews":
                review_project = (qs.get("project") or [""])[0] or None
                try:
                    self._send(200, service.reviews(review_project))
                except AppServiceError as exc:
                    self._send(400, {"error": str(exc), "package_id": PACKAGE_ID})
                return
            if path == "/v1/handoffs":
                handoff_project = (qs.get("project") or [""])[0] or None
                try:
                    self._send(200, service.handoffs(handoff_project))
                except AppServiceError as exc:
                    self._send(400, {"error": str(exc), "package_id": PACKAGE_ID})
                return
            if path == "/v1/brief":
