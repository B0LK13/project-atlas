...
def build_next_lens(vault: Path, project_id: str) -> dict[str, Any]:
    ...
    answer_evidence_stale = drift.get("status") == "STALE"
    live_source_unverified = str(drift.get("reason_code") or "") in {
        "SOURCE_ROOT_UNVERIFIED",
        "MANIFEST_ABSENT",
        "NO_ACTIVE_SOURCES",
    }
    if answer_evidence_stale:
        changed = [
        ...
        collected.append(
            _queue_item(
                kind="stale_evidence",
                title="Reconnect: live sources drifted from connect-manifest",
                why="What Next evidence is stale until atlas connect is re-run",
                action="Re-run atlas connect, then refresh What Next",
