"""AS-CODER-ALPHA-OVERVIEW-READ-001 -- vault-scoped Project Overview REPORT READ.

Read-only consume of persisted AS-CODER-ALPHA-OVERVIEW-001 answer artifacts
under generated/answers/ans-overview-*.json. This module never invokes
materialize_overview_lenses / build_overview_lens, never writes, and never
...
PACKAGE_ID: Final[str] = "AS-CODER-ALPHA-OVERVIEW-READ-001"
GENERATOR_ID: Final[str] = "atlas-coder-alpha-overview-read-001"
SCHEMA_ID: Final[str] = "atlas.coder-alpha.overview-read.v1"
SOURCE_PACKAGES: Final[tuple[str, ...]] = ("AS-CODER-ALPHA-OVERVIEW-001",)
SOURCE_RELATIVE_DIR: Final[str] = "generated/answers"
SOURCE_FILENAME_PREFIX: Final[str] = "ans-overview-"
SOURCE_FILENAME_SUFFIX: Final[str] = ".json"
SOURCE_COMMAND: Final[str] = "atlas overview-status"
TRUTH_BOUNDARY: Final[str] = (
    "OVERVIEW != AUTHORITY / UI != CANONICAL / EMPTY != HEALTHY / "
...
class WebOverviewReadError(ValueError):
    """Fail-closed Project Overview REPORT READ error."""


def _honesty() -> dict[str, bool | str]:
    return {
        "overview_is_authority": False,
...
def _resolve_vault(vault: Path) -> Path:
    ...
    try:
        ...
        raise WebOverviewReadError(f"overview-read-vault-unreadable:{exc}") from exc
    if not root.is_dir():
        raise WebOverviewReadError("overview-read-vault-missing")
    return root


def _inside(root: Path, candidate: Path) -> Path:
    if candidate.is_symlink():
        raise WebOverviewReadError("overview-read-symlink-forbidden")
    resolved = candidate.resolve()
    if not resolved.is_relative_to(root):
        raise WebOverviewReadError("overview-read-path-escape")
    return resolved
...
def _reject_invented_authority(payload: dict[str, Any], *, name: str) -> None:
    if payload.get("authentic_pilot") is True or payload.get("AUTHENTIC_PILOT") is True:
        raise WebOverviewReadError("overview-read-authentic-pilot-invented")
    if payload.get("MERGE_AUTHORIZATION") in {"GRANTED", "granted", True}:
        raise WebOverviewReadError(f"overview-read-merge-authority-invented:{name}")
    if (
        payload.get("overview_is_authority") is True
        or payload.get("lens_is_authority") is True
        or payload.get("ui_is_canonical") is True
    ):
        raise WebOverviewReadError("overview-read-authority-invented")
...
def _load_json_object(path: Path) -> dict[str, Any]:
    try:
        ...
        raise WebOverviewReadError(f"overview-read-artifact-invalid:{path.name}") from exc
    if not isinstance(payload, dict):
        raise WebOverviewReadError(f"overview-read-artifact-not-object:{path.name}")
...
def _existing_overview_answers(vault: Path) -> dict[str, Any]:
    ...
    if answers_dir.exists():
        ...
        if not answers_dir.is_dir():
            raise WebOverviewReadError("overview-read-answers-not-directory")
        ...
        for child in sorted(answers_dir.iterdir(), key=lambda path: path.name):
            ...
            if not child.is_file():
                raise WebOverviewReadError(f"overview-read-artifact-not-file:{name}")
            ...
            if declared and declared != project_id:
                raise WebOverviewReadError(
                    f"overview-read-project-mismatch:{name}:{declared}"
                )
            rows.append(
                {
                    "project_id": project_id,
...
def _rollup(view: dict[str, Any]) -> tuple[StatusRollup, str, str, bool]:
    ...
    if artifact_count == 0:
        return (
            "EMPTY",
            "no existing ans-overview artifacts; EMPTY != HEALTHY; "
            "OVERVIEW != AUTHORITY",
            "EMPTY_OVERVIEW_VIEW",
            False,
        )
    if malformed_count > 0:
        return (
            "UNKNOWN",
            "ans-overview artifacts exist but integrity is incomplete; "
            "UNKNOWN != HEALTHY; mixed valid+corrupt is not a healthy overview",
            "UNKNOWN_OVERVIEW_VIEW",
            False,
        )
    return (
        "PRESENT",
        "existing Project Overview answers projected; OVERVIEW != AUTHORITY; "
        "UI != CANONICAL",
        "OVERVIEW_VIEW_PROJECTED",
        True,
    )
...
def read_overview_view(vault: Path) -> dict[str, Any]:
    """Read-only consume of existing ans-overview artifacts. Never writes."""
    return _envelope(view=_existing_overview_answers(_resolve_vault(vault)))


def render_overview_status_text(view: dict[str, Any]) -> str:
    ...
    lines = [
        f"atlas overview-status [{view.get('status', 'UNKNOWN')}]",
        ...
        (
            "  honesty:          OVERVIEW != AUTHORITY; UI != CANONICAL; "
