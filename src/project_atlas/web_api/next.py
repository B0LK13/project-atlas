...
Never writes Layer B. NEXT LENS != AUTHORITY. NEXT ACTION != COMMAND.
"""
...
TRUTH_BOUNDARY = "NEXT LENS != AUTHORITY / NEXT ACTION != COMMAND / API != TRUTH CORE"
_ID_RE = re.compile(r"^[a-z][a-z0-9-]{0,63}$")


class WebNextError(ValueError):
    """Fail-closed next-lens API error."""
...
def read_project_next(vault: Path, project_id: str) -> dict[str, Any]:
    """Return project-scoped What Next lens (read-only, derived)."""
    token = _safe_project_id(project_id)
    try:
        lens = build_next_lens(vault, token)
