...
Read-only consume of persisted AS-CODER-ALPHA-NEXT-001 answer artifacts
under generated/answers/ans-next-*.json. This module never invokes
materialize_next_lenses / derive_next_lenses / build_next_lens, never
writes, and never treats the queue as a command or as authority.
...
PACKAGE_ID: Final[str] = "AS-CODER-ALPHA-NEXT-READ-001"
GENERATOR_ID: Final[str] = "atlas-coder-alpha-next-read-001"
SCHEMA_ID: Final[str] = "atlas.coder-alpha.next-read.v1"
SOURCE_PACKAGES: Final[tuple[str, ...]] = ("AS-CODER-ALPHA-NEXT-001",)
SOURCE_RELATIVE_DIR: Final[str] = "generated/answers"
SOURCE_FILENAME_PREFIX: Final[str] = "ans-next-"
SOURCE_FILENAME_SUFFIX: Final[str] = ".json"
SOURCE_COMMAND: Final[str] = "atlas next-status"
TRUTH_BOUNDARY: Final[str] = (
    "NEXT != AUTHORITY / NEXT != COMMAND / EMPTY != HEALTHY / "
    "UNKNOWN != HEALTHY / MCP != AUTHORITY / WRITE_APPLIED = false / "
    "NOT AS-2.0-NEXT-001 / src/project_atlas/atlas3/** UNTOUCHED / "
    "MERGE_AUTHORIZATION = NOT_GRANTED"
)
...
class WebNextReadError(ValueError):
    """Fail-closed What Next REPORT READ error."""
...
def _resolve_vault(vault: Path) -> Path:
    ...
    try:
        ...
        raise WebNextReadError(f"next-read-vault-unreadable:{exc}") from exc
    if not root.is_dir():
        raise WebNextReadError("next-read-vault-missing")
    return root


def _inside(root: Path, candidate: Path) -> Path:
    if candidate.is_symlink():
        raise WebNextReadError("next-read-symlink-forbidden")
    resolved = candidate.resolve()
    if not resolved.is_relative_to(root):
        raise WebNextReadError("next-read-path-escape")
    return resolved
...
def _reject_invented_authority(payload: dict[str, Any], *, name: str) -> None:
    if payload.get("authentic_pilot") is True or payload.get("AUTHENTIC_PILOT") is True:
        raise WebNextReadError("next-read-authentic-pilot-invented")
    if payload.get("MERGE_AUTHORIZATION") in {"GRANTED", "granted", True}:
        raise WebNextReadError(f"next-read-merge-authority-invented:{name}")
    if payload.get("next_is_authority") is True or payload.get("lens_is_authority") is True:
        raise WebNextReadError("next-read-authority-invented")
    if payload.get("next_is_command") is True or payload.get("auto_execution") is True:
        raise WebNextReadError("next-read-command-invented")
...
def _load_json_object(path: Path) -> dict[str, Any]:
    try:
        ...
        raise WebNextReadError(f"next-read-artifact-invalid:{path.name}") from exc
    if not isinstance(payload, dict):
        raise WebNextReadError(f"next-read-artifact-not-object:{path.name}")
...
def _existing_next_answers(vault: Path) -> dict[str, Any]:
    ...
    if answers_dir.exists():
        ...
        if not answers_dir.is_dir():
            raise WebNextReadError("next-read-answers-not-directory")
        ...
        for child in sorted(answers_dir.iterdir(), key=lambda path: path.name):
            ...
            if not child.is_file():
                raise WebNextReadError(f"next-read-artifact-not-file:{name}")
            ...
            if declared and declared != project_id:
                raise WebNextReadError(
                    f"next-read-project-mismatch:{name}:{declared}"
                )
            rows.append(
                {
                    "project_id": project_id,
...
def _rollup(view: dict[str, Any]) -> tuple[StatusRollup, str, str, bool]:
    ...
    if artifact_count == 0:
        return (
            "EMPTY",
            "no existing ans-next artifacts; EMPTY != HEALTHY; NEXT != AUTHORITY",
            "EMPTY_NEXT_VIEW",
            False,
        )
    if malformed_count > 0:
        return (
            "UNKNOWN",
            "ans-next artifacts exist but integrity is incomplete; "
            "UNKNOWN != HEALTHY; mixed valid+corrupt is not a healthy next lens",
            "UNKNOWN_NEXT_VIEW",
            False,
        )
    return (
        "PRESENT",
        "existing What Next answers projected; NEXT != AUTHORITY; NEXT != COMMAND",
        "NEXT_VIEW_PROJECTED",
        True,
    )
...
def read_next_view(vault: Path) -> dict[str, Any]:
    """Read-only consume of existing ans-next artifacts. Never writes."""
    return _envelope(view=_existing_next_answers(_resolve_vault(vault)))


def render_next_status_text(view: dict[str, Any]) -> str:
    ...
    lines = [
        f"atlas next-status [{view.get('status', 'UNKNOWN')}]",
        ...
        (
            "  honesty:          NEXT != AUTHORITY; NEXT != COMMAND; "
