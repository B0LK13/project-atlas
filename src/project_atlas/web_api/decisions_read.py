"""AS-CODER-ALPHA-DECISIONS-READ-001 -- vault-scoped Project Decisions REPORT READ.

Read-only consume of persisted AS-CODER-ALPHA-DECISIONS-001 answer artifacts
under generated/answers/ans-decisions-*.json. This module never invokes
materialize_decisions_lenses / build_decisions_lens, never writes, and never
treats the lens as Truth Core or as a live-estate current view.
...
PACKAGE_ID: Final[str] = "AS-CODER-ALPHA-DECISIONS-READ-001"
GENERATOR_ID: Final[str] = "atlas-coder-alpha-decisions-read-001"
SCHEMA_ID: Final[str] = "atlas.coder-alpha.decisions-read.v1"
SOURCE_PACKAGES: Final[tuple[str, ...]] = ("AS-CODER-ALPHA-DECISIONS-001",)
SOURCE_RELATIVE_DIR: Final[str] = "generated/answers"
SOURCE_FILENAME_PREFIX: Final[str] = "ans-decisions-"
SOURCE_FILENAME_SUFFIX: Final[str] = ".json"
SOURCE_COMMAND: Final[str] = "atlas decisions-status"
TRUTH_BOUNDARY: Final[str] = (
    "DECISIONS != AUTHORITY / MODEL PARAPHRASE != OWNER DECISION / EMPTY != HEALTHY / "
...
class WebDecisionsReadError(ValueError):
    """Fail-closed Project Decisions REPORT READ error."""
...
def _resolve_vault(vault: Path) -> Path:
    ...
    try:
        ...
        raise WebDecisionsReadError(f"decisions-read-vault-unreadable:{exc}") from exc
    if not root.is_dir():
        raise WebDecisionsReadError("decisions-read-vault-missing")
    return root


def _inside(root: Path, candidate: Path) -> Path:
    if candidate.is_symlink():
        raise WebDecisionsReadError("decisions-read-symlink-forbidden")
    resolved = candidate.resolve()
    if not resolved.is_relative_to(root):
        raise WebDecisionsReadError("decisions-read-path-escape")
    return resolved
...
def _reject_invented_authority(payload: dict[str, Any], *, name: str) -> None:
    if payload.get("authentic_pilot") is True or payload.get("AUTHENTIC_PILOT") is True:
        raise WebDecisionsReadError("decisions-read-authentic-pilot-invented")
    if payload.get("MERGE_AUTHORIZATION") in {"GRANTED", "granted", True}:
        raise WebDecisionsReadError(f"decisions-read-merge-authority-invented:{name}")
    if (
        payload.get("decisions_is_authority") is True
        or payload.get("lens_is_authority") is True
        or payload.get("model_is_owner_decision") is True
    ):
        raise WebDecisionsReadError("decisions-read-authority-invented")
...
def _load_json_object(path: Path) -> dict[str, Any]:
    try:
        ...
        raise WebDecisionsReadError(f"decisions-read-artifact-invalid:{path.name}") from exc
    if not isinstance(payload, dict):
        raise WebDecisionsReadError(f"decisions-read-artifact-not-object:{path.name}")
...
def _existing_decisions_answers(vault: Path) -> dict[str, Any]:
    ...
    if answers_dir.exists():
        ...
        if not answers_dir.is_dir():
            raise WebDecisionsReadError("decisions-read-answers-not-directory")
        ...
        for child in sorted(answers_dir.iterdir(), key=lambda path: path.name):
            ...
            if not child.is_file():
                raise WebDecisionsReadError(f"decisions-read-artifact-not-file:{name}")
            ...
            if declared and declared != project_id:
                raise WebDecisionsReadError(
                    f"decisions-read-project-mismatch:{name}:{declared}"
                )
            rows.append(
                {
                    "project_id": project_id,
...
def _rollup(view: dict[str, Any]) -> tuple[StatusRollup, str, str, bool]:
    ...
    if artifact_count == 0:
        return (
            "EMPTY",
            "no existing ans-decisions artifacts; EMPTY != HEALTHY; "
            "DECISIONS != AUTHORITY",
            "EMPTY_DECISIONS_VIEW",
            False,
        )
    if malformed_count > 0:
        return (
            "UNKNOWN",
            "ans-decisions artifacts exist but integrity is incomplete; "
            "UNKNOWN != HEALTHY; mixed valid+corrupt is not a healthy decisions",
            "UNKNOWN_DECISIONS_VIEW",
            False,
        )
    return (
        "PRESENT",
        "existing Project Decisions answers projected; DECISIONS != AUTHORITY; "
        "MODEL PARAPHRASE != OWNER DECISION",
        "DECISIONS_VIEW_PROJECTED",
        True,
    )
...
def read_decisions_view(vault: Path) -> dict[str, Any]:
    """Read-only consume of existing ans-decisions artifacts. Never writes."""
    return _envelope(view=_existing_decisions_answers(_resolve_vault(vault)))


def render_decisions_status_text(view: dict[str, Any]) -> str:
    ...
    lines = [
        f"atlas decisions-status [{view.get('status', 'UNKNOWN')}]",
        ...
        (
            "  honesty:          DECISIONS != AUTHORITY; MODEL PARAPHRASE != OWNER DECISION; "
