...
def d148_evidence_applies(
    ...
    if not current_fp or recorded_fp != current_fp:
        return False
    # D-149R6: a bound cert on a non-authentic/demo path is not current.
...
_ESTATE_CONSUME_STATUSES: Final[frozenset[str]] = frozenset(
    {"BLOCKED_OWNER", "BLOCKED_DEPENDENCY"}
)
    ...
    try:
        for node in nodes.values():
            if node.PACKAGE_ID not in AUTHENTIC_O2_PACKAGES:
                continue
            if node.status not in _ESTATE_CONSUME_STATUSES:
                continue
            ...
            if estate_prerequisite_consumable(
            ...
            # D-149R6: after a legitimate estate consume, compile/query may
            # still be waiting on prior O2 packages. A later bound ingest
            # cert must be allowed to promote BLOCKED_OWNER+NONE+empty-deps
            # to READY. This is not gate escalation: protected gates are
            # skipped above and CREDENTIAL remains until estate is consumed.
            if (
                sequential
                and not node.DEPENDENCIES
                and node.OWNER_GATE == "NONE"
