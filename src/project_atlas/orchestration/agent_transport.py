"""AS-ORCH-001D-R1 Cursor CLI process transport.

Logical Cursor identity is ``agent`` / ``cursor-agent``. Physical launchers
may be a direct executable or a trusted Windows ``.cmd`` wrapper. Prompt text
travels on stdin, never through ``cmd.exe`` command text.
...
MAX_RESULT_CANDIDATE_BYTES = 64 * 1024
READ_ONLY_CURSOR_MODE: Literal["ask"] = "ask"
READ_ONLY_CURSOR_FLAGS: tuple[str, ...] = (
    "--print",
    "--output-format",
    "json",
    "--mode",
    READ_ONLY_CURSOR_MODE,
)
FORBIDDEN_CURSOR_FLAGS = frozenset(
    {"--force", "--yolo", "--continue", "-" + "-resume"}
)
...
class ResolvedCursorExecutable(BaseModel):
    """Trusted logical Cursor identity bound to one physical launcher."""
...
class CursorLaunchPlan(BaseModel):
    """Deterministic launch facts. Not authority and not a prompt store."""
    ...
    cursor_mode: Literal["ask"] = READ_ONLY_CURSOR_MODE
    uses_force: Literal[False] = False


@dataclass(frozen=True, slots=True)
class ProcessRunRequest:
    """Trusted argv invocation. Prompt bytes travel on stdin, never a shell string."""
...
def extract_result_payload(stdout: bytes) -> object | None:
    """Return a candidate AgentResultEnvelope from Cursor JSON, or None.

    Used only as validated evidence. Never used to choose a route.
    """
    if len(stdout) > MAX_RESULT_CANDIDATE_BYTES:
        stdout = stdout[:MAX_RESULT_CANDIDATE_BYTES]
    text = stdout.decode("utf-8", errors="replace").strip()
    if not text:
        return None
    blobs: list[object] = []
    try:
        ...
        for line in text.splitlines():
            ...
            try:
                ...
                continue
    for item in reversed(blobs):
        candidate = _envelope_candidate(item)
        if candidate is not None:
            return candidate
    return None


def _envelope_candidate(payload: object) -> object | None:
    if isinstance(payload, list):
        for entry in reversed(payload):
            found = _envelope_candidate(entry)
            if found is not None:
                return found
        return None
    if not isinstance(payload, dict):
        return None
    if payload.get("schema_version") == 1 and "producer" in payload and "task" in payload:
        return payload
    result = payload.get("result")
    if isinstance(result, dict):
        return _envelope_candidate(result)
    if isinstance(result, str) and result.lstrip().startswith("{"):
        try:
            parsed: object = json.loads(result)
        except json.JSONDecodeError:
            return None
        return _envelope_candidate(parsed)
    return None
