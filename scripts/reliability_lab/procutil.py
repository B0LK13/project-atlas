"""Shared real-process spawn/snapshot/cleanup helpers for lab experiments.

DUPLICATION NOTICE: this is a deliberate, reconciled copy of PR #771's
``windows_conformance/procutil.py`` -- see this package's ``__init__.py``
docstring ("Duplication reconciliation") for why. Keep changes here in
sync with that module's intent; if they diverge for lab-specific reasons,
say so explicitly rather than letting it happen silently.

Every experiment that spawns a real OS process uses these so that process
cleanup is enforced structurally rather than trusted: a pre-spawn
snapshot, a post-cleanup snapshot, and an explicit leak count -- never
inferred from a command's exit code alone.
"""
...
def snapshot_pids(command_line_substring: str) -> set[int]:
    ...
    ps = (
        ...
        f"Where-Object {{ $_.CommandLine -like '*{command_line_substring}*' "
