...
def issue(root: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "receipt_id": "ASR-" + hashlib.sha256(str(state["session"]["session_id"]).encode()).hexdigest()[:16],
        "authority_role": "evidence-only",
        "is_authority": False,
        "receipt_is_authority": False,
        "session": state["session"],
        "agent": {**state["agent"], "adapter_readiness": state.get("preflight", {}).get("readiness")},
        "skill": {
            **state["skill"],
            "hash_matched": True,
            "verified": True,
        ...
        "rehearsal": {
            "rehearsal_id": "ARH-" + hashlib.sha256(str(state["session"]["session_id"]).encode()).hexdigest()[:16],
